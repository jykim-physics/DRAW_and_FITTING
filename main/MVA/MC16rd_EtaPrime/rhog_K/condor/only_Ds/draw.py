...
title_dict=dict()
title_dict['rhog_K']=r'$D^+_{(s)} \to (\eta^\prime \to \rho^0 \gamma) K^+$'

mass_dict=dict()
mass_dict['rhog_K']=r'$M(\eta^\prime K^+)$'
...
branches = ["Hp_dr", "Dp_dz", "Dp_cosAngleBetweenMomentumAndVertexVectorInXYPlane", "Dp_cosHelicityAngleMomentum", "Dp_CMS_p",
            "skimhad", "Dp_isSignal", "is_true_Ds","Dp_M"]
training_features = [
    "Hp_dr",
    "Dp_dz",
...
elements_sig = ["DpEtaPrimeK_Rhogamma"]
...
elements_sig = ["DspEtaPrimeK_Rhogamma"]
project_name = sig_MC_name
file_list_sig = []
for element in elements_sig:
    pattern = f"{base_path_sig}/{element}/{project_name}/*.root"
...
df_combined = df_combined_all_range.query('Dp_M>1.95 & Dp_M<1.98')
