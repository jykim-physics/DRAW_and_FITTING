...
title_dict=dict()
title_dict['pipipi_K']=r'$D^+_{(s)} \to (\eta^\prime \to \pi^+ \pi^- \eta_{3\pi}) K^+$'

mass_dict=dict()
mass_dict['pipipi_K']=r'$M(\eta^\prime K^+)$'
...
branches = ["Hp_dr", "Dp_dz", "Dp_cosAngleBetweenMomentumAndVertexVectorInXYPlane", "Dp_cosHelicityAngleMomentum", "Dp_CMS_p",
            "skimhad", "Dp_isSignal", "is_true_Ds","Dp_M", "pipipi_K_rank"]
training_features = [
    "Hp_dr",
    "Dp_dz",
...
df_bkg = df_bkg.query('pipipi_K_rank==1')
...
elements_sig = ["DpEtaPrimeK_Eta_3Pion"]
...
elements_sig = ["DspEtaPrimeK_Eta_3Pion"]
project_name = sig_MC_name
file_list_sig = []
for element in elements_sig:
    pattern = f"{base_path_sig}/{element}/{project_name}/*.root"
...
df_signal = df_signal.query('pipipi_K_rank==1')
...
df_bkg_apply = df_bkg_apply.query('pipipi_K_rank==1')
