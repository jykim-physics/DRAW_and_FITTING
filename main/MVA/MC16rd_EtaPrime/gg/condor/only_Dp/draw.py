...
title_dict=dict()
title_dict['gg']=r'$D^+_{(s)} \to (\eta^\prime \to \pi^+ \pi^- \eta_{\gamma\gamma}) \pi^+$'

mass_dict=dict()
mass_dict['gg']=r'$M(\eta^\prime \pi^+)$'
...
df_bkg = df_bkg.query('Dp_M> 1.6 & Dp_M<1.93')
...
elements_sig = ["DpEtaPrimePi_Eta_2Photon"]
...
elements_sig = ["DspEtaPrimePi_Eta_2Photon"]
project_name = sig_MC_name
file_list_sig = []
for element in elements_sig:
    pattern = f"{base_path_sig}/{element}/{project_name}/*.root"
...
df_signal = df_signal.query('Dp_M> 1.6 & Dp_M<1.93')
...
df_bkg_apply = df_bkg_apply.query('Dp_M> 1.6 & Dp_M<1.93')
...
plt.hist(df_signal['Dp_M'], bins=np.linspace(1.7, 1.93, 71), histtype='step', color='red', label='All')
...
# Histogram for signal
plt.hist(df_signal['Dp_M'], bins=np.linspace(1.7, 1.93, 71), histtype='step', color='blue', label='Before')

# Histogram for background
plt.hist(df_background['Dp_M'], bins=np.linspace(1.7, 1.93, 71), histtype='step', color='red', label='After')
