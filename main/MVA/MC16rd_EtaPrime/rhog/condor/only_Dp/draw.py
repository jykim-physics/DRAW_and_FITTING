...
title_dict=dict()
title_dict['rhog']=r'$D^+_{(s)} \to (\eta^\prime \to \rho^0 \gamma) \pi^+$'

mass_dict=dict()
mass_dict['rhog']=r'$M(\eta^\prime \pi^+)$'
...
elements_sig = ["DpEtaPrimePi_Rhogamma"]
...
elements_sig = ["DspEtaPrimePi_Rhogamma"]
project_name = sig_MC_name
file_list_sig = []
for element in elements_sig:
    pattern = f"{base_path_sig}/{element}/{project_name}/*.root"
