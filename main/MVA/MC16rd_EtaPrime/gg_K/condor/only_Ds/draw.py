...
title_dict=dict()
title_dict['gg_K']=r'$D^+_{(s)} \to (\eta^\prime \to \pi^+ \pi^- \eta_{\gamma\gamma}) K^+$'

mass_dict=dict()
mass_dict['gg_K']=r'$M(\eta^\prime K^+)$'

#gen_MC_name = "/share/storage/jykim/storage_b2/storage/reduced_ntuples/MC16rd/PrimeHp/MC16rd_loose_v2/PrimeHp"
gen_MC_name = "/share/storage/jykim/storage_b2/storage/reduced_ntuples/MC16rd/PrimeHp/MC16rd_loose_v2_new/PrimeHp"
base_path_sig = "/share/storage/jykim/storage_ghi/Ntuples_ghi_2/MC16rd_sigMC"
sig_MC_name = "260907_loose_v2_all_const_on_3pion_bcs"
tree_name="gg_K"

overall_version = "loose_v2_train_v0"
...
cm_elements = [""]
#branches = ['Hp_mcPDG','skimhad','Dp_dz','Hp_binaryP','Hp_pionID','Dp_Psum','dM_pi0_mask_nonan','dM_pi0_mask_2_nonan','ip_genMotherID','etapip_Eta_genMotherID','Pip_genMotherPDG','etapip_Eta_genMotherPDG',  'Dp_cosHelicityAngleMomentum','Pip_dr',"Dp_isSignal","Dp_cosAngleBetweenMomentumAndVertexVectorInXYPlane",
#            "Pip_p","Dp_CMS_p","etapip_Eta_Easym","etapip_Eta_p","Dp_M","etapip_Eta_daughterDiffOfPhi_0_1","etapip_Eta_daughterAngle_0_1"]
branches = ["Hp_dr", "Dp_dz", "Dp_cosAngleBetweenMomentumAndVertexVectorInXYPlane", "Dp_cosHelicityAngleMomentum", "Dp_CMS_p",
            "skimhad", "Dp_isSignal", "is_true_Ds","Dp_M", "Eta_daughterDiffOf_0_1_phi", "Eta_daughterAngle_0_1"]
training_features = [
    "Hp_dr",
    "Dp_dz",
...
df_bkg = df_bkg.query('is_true_Ds!=1')
df_bkg = df_bkg.query('Dp_M> 1.9 & Dp_M<2.1')
df_bkg = df_bkg.query('skimhad==1')
#df_bkg = df_bkg.query('abs(Eta_daughterDiffOf_0_1_phi)<1.8')
#df_bkg = df_bkg.query('Eta_daughterAngle_0_1<1.6')
...
elements_sig = ["DpEtaPrimeK_Eta_2Photon"]
...
elements_sig = ["DspEtaPrimeK_Eta_2Photon"]
project_name = sig_MC_name
file_list_sig = []
for element in elements_sig:
    pattern = f"{base_path_sig}/{element}/{project_name}/*.root"
...
df_signal_Dsp = df_signal_Dsp.query('is_true_Ds==1')

#unique_values = df_signal_Dsp['etapip_Eta_genMotherPDG'].unique()
#print(unique_values)
df_signal = pd.concat([df_signal_Dsp, df_signal_Dsp], ignore_index=True)
#df_signal = df_signal.query('Pip_p>0.4 ')
df_signal = df_signal.query('Dp_M> 1.9 & Dp_M<2.1')
#df_signal = df_signal.query('abs(Eta_daughterDiffOf_0_1_phi)<1.8')
#df_signal = df_signal.query('Eta_daughterAngle_0_1<1.6')
...
log_scale_features = {'Hp_dr', 'Dp_dz', 'Dp_cosAngleBetweenMomentumAndVertexVectorInXYPlane'}
...
df_bkg_apply = df_bkg_apply.query('Dp_M> 1.9 & Dp_M<2.1')
#df_bkg_apply = df_bkg_apply.query('abs(Eta_daughterDiffOf_0_1_phi)<1.8')
#df_bkg_apply = df_bkg_apply.query('Eta_daughterAngle_0_1<1.6')
...
df_isSignal_true = df_bkg_apply.query('Dp_isSignal==1 or is_true_Ds==1')
df_isSignal_false = df_bkg_apply.query('Dp_isSignal!=1 and is_true_Ds!=1')
...
df_combined = df_combined_all_range.query('Dp_M>1.93 & Dp_M<1.99')
...
def figure_of_merit(df, cut_value):
    ...
    Nsig = (df_cut['is_true_Ds'] == 1).sum()
    Nbkg = (df_cut['is_true_Ds'] != 1).sum()
    if Nsig + Nbkg == 0:
        return 0
...
plt.hist(df_signal['Dp_M'], bins=np.linspace(1.9, 2.05, 71), histtype='step', color='red', label='All')
...
print(f'Dp nsig={len(df_signal_matched_true)}')
#print(f'nsig={len(df_signal_matched_true)*scale}')
df_signal_matched_true = df_signal.query('is_true_Ds==1')
print(f'Ds nsig={len(df_signal_matched_true)}')
...
# Histogram for signal
plt.hist(df_signal['Dp_M'], bins=np.linspace(1.9, 2.05, 71), histtype='step', color='blue', label='Before')

# Histogram for background
plt.hist(df_background['Dp_M'], bins=np.linspace(1.9, 2.05, 71), histtype='step', color='red', label='After')
...
df_background_Ds = df_combined_all_range.query('is_true_Ds==1')
#df_background_Ds = df_combined_all_range.query('(etapip_Eta_isSignal == 1) & (Pip_genMotherID == etapip_Eta_genMotherID) & ((Pip_genMotherPDG == 431 and Pip_mcPDG == 211) | (Pip_genMotherPDG == -431 and Pip_mcPDG == -211) )')
df_background = df_combined_all_range.query('Dp_isSignal!=1 & is_true_Ds!=1')
#df_background = df_combined_all_range.query('(Dp_isSignal != 1) and ((etapip_Eta_isSignal != 1) or (Pip_genMotherID != etapip_Eta_genMotherID) or ((Pip_genMotherPDG != 431 or Pip_mcPDG != 211) and (Pip_genMotherPDG != -431 or Pip_mcPDG != -211) ) )')
