...
title_dict=dict()
title_dict['pipipi']=r'$D^+_{(s)} \to (\eta^\prime \to \pi^+ \pi^- \eta_{3\pi}) \pi^+$'

mass_dict=dict()
mass_dict['pipipi']=r'$M(\eta^\prime \pi^+)$'
...
branches = ["Hp_dr", "Dp_dz", "Dp_cosAngleBetweenMomentumAndVertexVectorInXYPlane", "Dp_cosHelicityAngleMomentum", "Dp_CMS_p",
            "skimhad", "Dp_isSignal", "is_true_Ds","Dp_M", "pipipi_rank"]
training_features = [
    "Hp_dr",
    "Dp_dz",
...
df_bkg = df_bkg.query('pipipi_rank==1')
...
elements_sig = ["DpEtaPrimePi_Eta_3Pion"]
...
elements_sig = ["DspEtaPrimePi_Eta_3Pion"]
project_name = sig_MC_name
file_list_sig = []
for element in elements_sig:
    pattern = f"{base_path_sig}/{element}/{project_name}/*.root"
...
df_signal = df_signal.query('pipipi_rank==1')
...
df_bkg_apply = df_bkg_apply.query('pipipi_rank==1')
