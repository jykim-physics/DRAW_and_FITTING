...
#suffix = "sumw2fixed"
file_name_Dp = f"Dp_etaeta_pipi_untag.png"
file_name_Dm = f"Dm_etaeta_pipi_untag.png"
file_name_Dall = f"Dall_etaeta_pipi_untag.png"
fitresult_name = f"etaeta_pipi_untag.result.root"
fitresult_text = f"etaeta_pipi_untag.txt"
file_sweight = f"etaeta_pipi_untag_sweight.root"
...
fit_var_name = "M(#eta_{3#pi}#eta_{3#pi}) [GeV/c^{2}]"
#fit_range = (1.60, 2.1)
fit_range = (1.70, 2.0)
#fit_range = (1.74, 2.0)
...
scale=1
N_scale=1
N_total = RooRealVar("N_total", "N_total (N_D+ + N_D-)", 5000*scale*N_scale, 100*scale*N_scale, 50000*scale*N_scale)  # N_total = N_D+ + N_D-

Nbkg_total = ROOT.RooRealVar("Nbkg_total", "Number of background events for D+", 10000*scale*N_scale, 500*scale*N_scale,400000*scale*N_scale)

mean = ROOT.RooRealVar("mean", "mean", 1.87, 1.84, 1.88)  # Central value
sigma = ROOT.RooRealVar("sigma", "sigma", 0.002, 0.00001, 0.1)  # Width parameter
gamma = ROOT.RooRealVar("gamma", "gamma", 0.1, -8.0, 8.0)  # Skewness parameter
...
fit_result = model_D_all.fitTo(data, RooFit.Save(), RooFit.Extended(True), RooFit.SumW2Error(True), ROOT.RooFit.NumCPU(8), RooFit.Strategy(1), ROOT.RooFit.Offset(False))
...
frame_D_all.GetXaxis().SetTitle("M(#eta_{3#pi}#eta_{3#pi}) [GeV/c^{2}]")
