...
def main():
    if len(sys.argv) < 2:
        print("ERROR: Please provide the path to the text file.\n"
              "Usage: python plot_bdt_scan.py path/to/your_text.txt")
        sys.exit(1)
    ...
    # Force x axis to [0.70, 0.99] as requested
    #plt.xlim(0.70, 0.99)
    plt.xlim(0.61, 0.90)
    ...
    if pd.notna(y_max):
        plt.annotate(f"max @ {x_max:.2f}",
                     ...
                     xytext=(x_max * 1.005, y_max * 0.97),
                     arrowprops=dict(arrowstyle="->"))

    png_path = "ntot_over_sqrt_uncertainty.png"
    plt.tight_layout()
