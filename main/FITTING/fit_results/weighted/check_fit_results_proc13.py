...
output_file = f"fitv6_results_proc13_{Kp_or_pip}_{gg_or_pipipi}.txt"
...
for root_file in os.listdir(input_dir):
    #if fnmatch.fnmatch(root_file, f"MC15rd_eta{Kp_or_pip}_{gg_or_pipipi}_fit_opt_loose_v7_fitv5_bdt_train_Dp_CMS_p_all_0.*weighted.root"):
    if fnmatch.fnmatch(root_file, f"proc13_eta{Kp_or_pip}_{gg_or_pipipi}_fit_opt_loose_v7_fitv6_bdt_train_Dp_CMS_p_all_0.*.root"):
        root_files.append(root_file)

root_files.sort()

with open(output_file, 'w') as out_f:
    out_f.write("File Name\tFit Status\tAcp_blind ± Uncertainty\tAcp_Ds_blind ± Uncertainty\tN_total ± Uncertainty\tN_total_Ds ± Uncertainty\tacp_ratio\tacp_ds_ratio\tn_total_ratio\tn_total_ds_ratio\tnbkg_total\tnbkg_total_error\n")
