...
def parse_text(raw_text: str) -> pd.DataFrame:
    ...
    line_re = re.compile(r'^proc13_.*$', re.MULTILINE)
    ...
    for m in line_re.finditer(raw_text):
        ...
        n_total = float(pairs[2][0])
        n_total_unc = float(pairs[2][1])

        acp_unc = float(pairs[0][1])
        acp_Ds_unc = float(pairs[1][1])
        ...
        records.append(
            {"bdt_cut": bdt_cut, "N_total": n_total,
             ...
             "acp_unc": acp_unc, "acp_Ds_unc": acp_Ds_unc}
        )
...
def main():
    ...
    #csv_path = f"ntot_over_sqrt_uncertainty_{tree_name}_proc13.csv"
    csv_path = f"acp_uncertainty_{tree_name}_proc13.csv"
    df.to_csv(csv_path, index=False)
    print(f"Saved CSV -> {csv_path}")

    # Plot
    plt.figure(figsize=(9, 5.5))
    plt.scatter(df["bdt_cut"], df["acp_unc"], marker="x")
    #plt.scatter(df["bdt_cut"], df["N_over_sigma"], marker="x")
    plt.xlabel("BDT cut value")
    plt.ylabel(r"$A_{CP}(D^+)$ uncertainty")
    #plt.ylabel(r"$N_{D^+_{\text{sig}}} / \sigma_{N_{D^+_{\text{sig}}}}$")
    ...
    if tree_name=='etapip_gg' or tree_name=='etapip_pipipi':
      plt.xlim(0.61, 0.90)
    elif tree_name=='etapip_gg_K' or tree_name=='etapip_pipipi_K':
      plt.xlim(0.70, 0.99)

    # Annotate the maximum point (if non-NaN)
    #idx_max = df["N_over_sigma"].idxmax()
    idx_max = df["acp_unc"].idxmin()
    x_max = df.loc[idx_max, "bdt_cut"]
    #y_max = df.loc[idx_max, "N_over_sigma"]
    y_max = df.loc[idx_max, "acp_unc"]
    if pd.notna(y_max):
        #plt.annotate(f"max @ {x_max:.2f}",
        plt.annotate(f"min @ {x_max:.2f}",
                     xy=(x_max, y_max),
                     xytext=(x_max, y_max * 1.05),
                     #xytext=(x_max * 1.005, y_max * 0.90),
                     arrowprops=dict(arrowstyle="->"))

    #png_path = f"ntot_over_sqrt_uncertainty_{tree_name}_proc13.png"
    png_path = f"acp_uncertainty_{tree_name}_proc13.png"
    plt.tight_layout()
