...
def main():
    ...
    csv_path = f"Ds_acp_uncertainty_{tree_name}_proc13.csv"
    df.to_csv(csv_path, index=False)
    print(f"Saved CSV -> {csv_path}")

    # Plot
    plt.figure(figsize=(9, 5.5))
    plt.scatter(df["bdt_cut"], df["acp_Ds_unc"], marker="x")
    #plt.scatter(df["bdt_cut"], df["N_over_sigma"], marker="x")
    plt.xlabel("BDT cut value")
    plt.ylabel(r"$A_{CP}(D^+_s)$ uncertainty")
    ...
    png_path = f"Ds_acp_uncertainty_{tree_name}_proc13.png"
    plt.tight_layout()
