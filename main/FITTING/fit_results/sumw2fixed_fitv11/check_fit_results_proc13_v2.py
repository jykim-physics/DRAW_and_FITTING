...
import fnmatch
import sys
gg_or_pipipi = sys.argv[1]
Kp_or_pip = sys.argv[2]

#gg_or_pipipi = "gg"
#gg_or_pipipi = "pipipi"
#Kp_or_pip = "pip"
input_dir = f"/share/storage/jykim/plots/proc_all/eta{Kp_or_pip}/{gg_or_pipipi}/generic/fitresult/"
output_file = f"fitv11_results_proc13_{Kp_or_pip}_{gg_or_pipipi}.txt"
...
for root_file in os.listdir(input_dir):
    if fnmatch.fnmatch(root_file, f"proc13_etapip_{gg_or_pipipi}_fit_opt_loose_v7_fitv11_bdt_train_Dp_CMS_p_all_0.*sumw2fixed.root"):
...
with open(output_file, 'w') as out_f:
    ...
    for root_file in root_files:
        ...
        f.Close()
    out_f.write(f"File with highest N_total/Uncertainty: {max_n_total_file} with ratio {max_n_total_ratio}\n")
    out_f.write(f"File with highest N_total_Ds/Uncertainty: {max_n_total_ds_file} with ratio {max_n_total_ds_ratio}\n")
    out_f.write(f"File with lowest Acp_blind uncertainty: {min_acp_file} with uncertainty {min_acp_error} (Acp_blind = {min_acp})\n")
    out_f.write(f"File with lowest Acp_Ds_blind uncertainty: {min_acp_ds_file} with uncertainty {min_acp_ds_error} (Acp_Ds_blind = {min_acp_ds})\n")
    out_f.write(f"Results saved in {output_file}")
    ...
    print(f"File with lowest Acp_blind uncertainty: {min_acp_file} with uncertainty {min_acp_error} (Acp_blind = {min_acp})")
    print(f"File with lowest Acp_Ds_blind uncertainty: {min_acp_ds_file} with uncertainty {min_acp_ds_error} (Acp_Ds_blind = {min_acp_ds})")
