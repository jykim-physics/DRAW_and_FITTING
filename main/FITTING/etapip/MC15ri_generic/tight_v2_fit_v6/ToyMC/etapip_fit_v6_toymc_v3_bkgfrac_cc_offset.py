...
bkg_frac = ROOT.RooRealVar("bkg_frac", "c0",0.5, 0, 1)
comb_rhopeta_model = ROOT.RooAddPdf("comb_rhopeta_model", "x_model", ROOT.RooArgSet(bkg_model, rhopeta_model), ROOT.RooArgSet(bkg_frac))
...
extended_model = ROOT.RooAddPdf("extended_model", "x_model", ROOT.RooArgSet(sig_model, Ds_model, comb_rhopeta_model), ROOT.RooArgSet(nsig, nDs, nbkg1))
# Step 1: Create the negative log-likelihood (NLL)
nll = extended_model.createNLL(data, ROOT.RooFit.Extended(True), ROOT.RooFit.NumCPU(15), ROOT.RooFit.Offset(True))
#nll = extended_model.createNLL(data, ROOT.RooFit.Extended(True), ROOT.RooFit.NumCPU(15))
...
pull_canvas.SaveAs("pull_distribution_nsig_5000_bkgfrac_cc.pdf")
...
nsig_hist.Draw()
nsig_canvas.SaveAs("nsig_distribution_5000_bkgfrac_cc.pdf")
...
nsig_error_canvas.SaveAs("nsig_error_distribution_5000_bkgfrac_cc.pdf")
