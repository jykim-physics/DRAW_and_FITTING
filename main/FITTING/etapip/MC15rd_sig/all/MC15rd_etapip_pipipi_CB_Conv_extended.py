...
ROOT.SetBelle2Style()
file_name = "/share/storage/jykim/plots/MC15rd/etapip/pipipi/MC15rd_6M_etapip_pipipi_Dp_M_opt_v2_CB_conv.png"
result_name = "/share/storage/jykim/plots/MC15rd/etapip/pipipi/MC15rd_6M_etapip_pipipi_Dp_M_opt_v2_CB_conv_result.txt"
...
mychain.Add("/share/storage/jykim/storage_ghi/Ntuples_ghi_2/MC15rd_sigMC/Dptoetapip_pipipi/241114_loose_v2/etapip_pipipi/*BCS.root")

tree_name_cc = "etapip_pipipi"
mychain_cc = ROOT.TChain(tree_name_cc)
mychain_cc.Add("/share/storage/jykim/storage_ghi/Ntuples_ghi_2/MC15rd_sigMC/Dptoetapip_pipipi_cc/241114_loose_v2/etapip_pipipi/*BCS.root")
...
CB_left = ROOT.RooFFTConvPdf("CB_left", "Convolution of Johnson and Gaussian", x, CB, gaussian)

nsig = ROOT.RooRealVar("nsig", "Number of signal events", 100000, 0, 1e6)
nbkg = ROOT.RooRealVar("nbkg", "Number of background events", 1000, 0, 1e5)
...
polynomial = ROOT.RooPolynomial("polynomial", "polynomial", x, ROOT.RooArgList(a0))

# Combine the two PDFs
fraction = ROOT.RooRealVar("fraction", "fraction", 0.5, 0.0, 1.0)
model = ROOT.RooAddPdf("model", "model", ROOT.RooArgList(CB_left, polynomial),  ROOT.RooArgList(nsig, nbkg))
#model = CB_lef

# Perform the fit
result = model.fitTo(data, ROOT.RooFit.Range(fit_range[0], fit_range[1]), ROOT.RooFit.NumCPU(4), ROOT.RooFit.Save(), ROOT.RooFit.Extended(True))
result.Print()
...
model.plotOn(frame, ROOT.RooFit.Name("Bkg"),ROOT.RooFit.Components("polynomial"), ROOT.RooFit.LineStyle(ROOT.kDashed), ROOT.RooFit.LineColor(ROOT.kGreen))
