import ROOT
from ROOT import RooFit, RooRealVar, RooDataSet, RooArgList, RooAddPdf, RooGaussian, RooFormulaVar, RooSimultaneous, RooCategory, RooStats
...
truth_var = ROOT.RooRealVar(truth_var, truth_var, 0, 100)

before_data = ROOT.RooDataSet("data","", mychain, ROOT.RooArgSet(x,Pip_charge,Pip_p, truth_var), cuts_Dp)
...
for i in file_list:
    mychain_cc.Add(i)
before_data_cc = ROOT.RooDataSet("data","", mychain_cc, ROOT.RooArgSet(x,Pip_charge, Pip_p, truth_var), cuts_Dm)
...
Nbkg_total = ROOT.RooRealVar("Nbkg_total", "Number of background events for D+", 250000*scale, 100000*scale, 400000*scale)
...
Ds_sigmaL = ROOT.RooRealVar("Ds_sigmaL", "sigma", 0.004496587011731394)
Ds_sigmaR = ROOT.RooRealVar("Ds_sigmaR", "sigma", 0.0007895868747757817)
Ds_alphaL = ROOT.RooRealVar("Ds_alphaL", "alphaL", 1.4415169793533602)
Ds_nL = ROOT.RooRealVar("Ds_nL", "nL", 2.27964910398667)
Ds_alphaR = ROOT.RooRealVar("Ds_alphaR", "alphaR", 0.5636318317672805)
Ds_nR = ROOT.RooRealVar("Ds_nR", "nR", 2.0844523589972743)
...
Ds_sigma_gaussian = ROOT.RooRealVar("Ds_sigma_gaussian", "sigma of Gaussian", 0.004, 0.0001, 0.01)
...
bkg_frac = ROOT.RooRealVar("bkg_frac", "fraction of Gaussian in BKG", 0.4, 0.1, 1)
...
data_combined = RooDataSet("data_combined", "Combined data", RooArgList(x, w_1, Pip_p, truth_var), RooFit.Index(cat),
...
N_total_Ds_sWeight = sPlot.GetYieldFromSWeight("N_total_Ds")
...
print(f"Yield N_total_Ds is {N_total_Ds.getVal()}. From sWeights it is {N_total_Ds_sWeight}")
...
for i in range(0,10):
    ...
    N_total_Ds_sWeight = sPlot.GetSWeight(i, "N_total_Ds")
    print(f"Entry {i}: N_total_Ds sWeight = {N_total_Ds_sWeight}")
...
# Open a new ROOT file (or overwrite if it already exists)
output_file = ROOT.TFile("output_file.root", "RECREATE")

# Write the data_combined object to the file
data_combined.Write()

# Close the ROOT file
output_file.Close()

print("Data saved to 'output_file.root'")

from ROOT import TFile, TTree

# Step 1: Extract the TTree from the RooDataSet
tree = data_combined.GetClonedTree()

tree.SetName("etapip_gg_sw")  # Replace "customTreeName" with the name you want


# Step 2: Create a new ROOT file
output_file = TFile("output_data_combined.root", "RECREATE")

# Step 3: Write the TTree to the new ROOT file
tree.Write()

# Step 4: Close the ROOT file
output_file.Close()

print("Data has been saved to output_data_combined.root")


"""
cdata = ROOT.TCanvas("sPlot", "sPlot demo", 800, 600)
#cdata.Divide(1, 2)
#cdata.cd(0)

#sdata_Pip_p = ROOT.RooDataSet(data_combined.GetName(), data_combined.GetTitle(), data_combined, data_combined.get(), "", "N_total_sw")

Pip_p_frame = Pip_p.frame(0.4, 5.4, 50)
#w_1.setVal
w_Dp_sw = ROOT.RooRealVar('w_Dp_sw', 'w_Dp_sw', 0,1)
sdata_Pip_p = ROOT.RooDataSet(data_combined.GetName(), data_combined.GetTitle(), data_combined, data_combined.get(), "", )
#sdata_Pip_p = ROOT.RooDataSet(data_combined.GetName(), data_combined.GetTitle(), data_combined, data_combined.get(), "", RooFit.WeightVar('w_1'))
for i in range(data_combined.numEntries()):
    event = data_combined.get(i)
    tmp_N_total_sw = event.getRealValue("N_total_sw")
    event.setWeight(tmp_N_total_sw)
    #w_Dp_sw.setVal(tmp_N_total_sw)
    #tmp_N_total_sw = sPlot.GetSWeight(i, "N_total")
    #tmp_N_total_sw = data_combined(i, "N_total_sw")
    tmp_w_1 = event.getRealValue("w_1")
    #w_1.setVal( tmp_w_1 * tmp_N_total_sw)
    print(f"tmp_w_1 : {tmp_w_1}")
    #w_Dp_sw.setVal(tmp_w_1 * tmp_N_total_sw)
    #w_Dp_sw.setVal(tmp_N_total_sw)
    #data_combined.add()

sdata_Pip_p.plotOn(Pip_p_frame, ROOT.RooFit.DataError(ROOT.RooAbsData.SumW2))
#data_combined.plotOn(Pip_p_frame, ROOT.RooFit.DataError(ROOT.RooAbsData.SumW2))
slicedData_Dp_true = data_combined.reduce(Cut="Dp_isSignal==1")
slicedData_Dp_true.plotOn(Pip_p_frame, ROOT.RooFit.DataError(ROOT.RooAbsData.SumW2))
Pip_p_frame.GetYaxis().SetRangeUser(0, Pip_p_frame.GetMaximum())
Pip_p_frame.SetTitle("sWeighted pion momentum")
Pip_p_frame.Draw()
cdata.SaveAs("test.png")

cdata_Ds = ROOT.TCanvas("sPlot", "sPlot demo", 800, 600)

sdata_Pip_p_Ds = ROOT.RooDataSet(data_combined.GetName(), data_combined.GetTitle(), data_combined, data_combined.get(), "", "N_total_Ds_sw")

Pip_p_frame_Ds = Pip_p.frame(0.4, 5.4, 50)

sdata_Pip_p_Ds.plotOn(Pip_p_frame_Ds, ROOT.RooFit.DataError(ROOT.RooAbsData.SumW2))
data_combined.plotOn(Pip_p_frame_Ds, Name="data")
Pip_p_frame_Ds.GetYaxis().SetRangeUser(0, Pip_p_frame_Ds.GetMaximum())
Pip_p_frame_Ds.SetTitle("sWeighted pion momentum")
Pip_p_frame_Ds.Draw()
cdata_Ds.SaveAs("test_Ds.png")
"""
