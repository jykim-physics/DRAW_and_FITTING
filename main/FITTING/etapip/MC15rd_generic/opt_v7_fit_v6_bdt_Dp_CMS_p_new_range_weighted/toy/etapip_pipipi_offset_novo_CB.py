...
file_name_Dp = f"/share/storage/jykim/plots/MC15rd/etapip/pipipi/generic/MC15rd_etapip_pipipi_fit_opt_loose_v7_fitv6_bdt_{args.train}_Dp_CMS_{args.sign}_{BDT_cut}_weighted.png"
file_name_Dm = f"/share/storage/jykim/plots/MC15rd/etapip/pipipi/generic/MC15rd_etapip_pipipi_fit_opt_loose_v7_fitv6_bdt_{args.train}_Dm_CMS_{args.sign}_{BDT_cut}_weighted.png"
fitresult_name = f"/share/storage/jykim/plots/MC15rd/etapip/pipipi/generic/fitresult/MC15rd_etapip_pipipi_fit_opt_loose_v7_fitv6_bdt_{args.train}_{args.sign}_{BDT_cut}_weighted.root"
fitresult_text = f"/share/storage/jykim/plots/MC15rd/etapip/pipipi/generic/fitresult/MC15rd_etapip_pipipi_fit_opt_loose_v7_fitv6_bdt_{args.train}_{args.sign}_{BDT_cut}_weighted.txt"
...
#N_total = RooRealVar("N_total", "N_total (N_D+ + N_D-)", 90000*scale*N_scale,  30000*scale*N_scale, 160000*scale*N_scale)  # N_total = N_D+ + N_D-
N_total = RooRealVar("N_total", "N_total (N_D+ + N_D-)", 60000*scale*N_scale,  10000*scale*N_scale, 300000*scale*N_scale)  # N_total = N_D+ + N_D-
#N_total = RooRealVar("N_total", "N_total (N_D+ + N_D-)", 50000*scale*N_scale,  10000*scale*N_scale, 80000*scale*N_scale)  # N_total = N_D+ + N_D-
...
#N_total_Ds = RooRealVar("N_total_Ds", "N_total (N_Ds+ + N_Ds-)", 80000*scale*N_scale,  30000*scale*N_scale,400000*scale*N_scale)  # N_total = N_D+ + N_D-
N_total_Ds = RooRealVar("N_total_Ds", "N_total (N_Ds+ + N_Ds-)", 120000*scale*N_scale,  20000*scale*N_scale,500000*scale*N_scale)  # N_total = N_D+ + N_D-
#N_total_Ds = RooRealVar("N_total_Ds", "N_total (N_Ds+ + N_Ds-)", 100000*scale*N_scale,  20000*scale*N_scale,150000*scale*N_scale)  # N_total = N_D+ + N_D-
#Acp_Ds = RooRealVar("Acp_Ds", "Acp", 0, -0.5, 0.5)  # A_Cp as a fit parameter
...
#Nbkg_total = ROOT.RooRealVar("Nbkg_total", "Number of background events for D+", 100000*scale*N_scale, 50000*scale*N_scale,600000*scale*N_scale)
Nbkg_total = ROOT.RooRealVar("Nbkg_total", "Number of background events for D+", 250000*scale*N_scale, 20000*scale*N_scale,600000*scale*N_scale)
#Nbkg_total = ROOT.RooRealVar("Nbkg_total", "Number of background events for D+", 80000*scale*N_scale, 30000*scale*N_scale,180000*scale*N_scale)
...
sigmaL = ROOT.RooRealVar("sigmaL", "sigmaL",  0.005118646351967811)
sigmaR = ROOT.RooRealVar("sigmaR", "sigmaR",  0.00015351473767328484)
alphaL = ROOT.RooRealVar("alphaL", "alphaL", 1.6482980744642557)
nL = ROOT.RooRealVar("nL", "nL",  1.922241613600479)
alphaR = ROOT.RooRealVar("alphaR", "alphaR", 0.12624613320969016)
nR = ROOT.RooRealVar("nR", "nR", 1.9554463224265144)
...
sigma_gaussian = ROOT.RooRealVar("sigma_gaussian", "sigma of Gaussian", 0.006, 0.001, 0.01)
...
Ds_sigmaL = ROOT.RooRealVar("Ds_sigmaL", "sigma", 0.0010550975580082191)
Ds_sigmaR = ROOT.RooRealVar("Ds_sigmaR", "sigma", 0.001169085647673563)
Ds_alphaL = ROOT.RooRealVar("Ds_alphaL", "alphaL", 0.5234095463143262)
Ds_nL = ROOT.RooRealVar("Ds_nL", "nL", 2.5165086971577844)
Ds_alphaR = ROOT.RooRealVar("Ds_alphaR", "alphaR", 0.868516043831275)
Ds_nR = ROOT.RooRealVar("Ds_nR", "nR", 2.16058070496527)
...
Ds_sigma_gaussian = ROOT.RooRealVar("Ds_sigma_gaussian", "sigma of Gaussian", 0.006, 0.001, 0.01)
...
novo_sigma = ROOT.RooRealVar("novo_sigma", "Sigma", 0.05532264565360561)
novo_tail = ROOT.RooRealVar("novo_tail", "Tail", 0.25942452142670464)
...
ToyMC_all = ROOT.RooMCStudy(sim_model, {x,cat}, Extended(True), FitOptions(Save(True),PrintEvalErrors(0),PrintLevel(1), NumCPU(28), Offset("initial")))
