...
file_name_Dp = "/share/storage/jykim/plots/MC15rd/etaKp/pipipi/generic/MC15rd_etapip_pipipi_K_fit_opt_loose_v7_fitv2_Dp.png"
file_name_Dm = "/share/storage/jykim/plots/MC15rd/etaKp/pipipi/generic/MC15rd_etapip_pipipi_K_fit_opt_loose_v7_fitv2_Dm.png"
fitresult_name = "/share/storage/jykim/plots/MC15rd/etaKp/pipipi/generic/fitresult/MC15rd_etapip_pipipi_K_fit_opt_loose_v7_fitv2.root"
fitresult_text = "/share/storage/jykim/plots/MC15rd/etaKp/pipipi/generic/fitresult/MC15rd_etapip_pipipi_K_fit_opt_loose_v7_fitv2.txt"
...
N_total = RooRealVar("N_total", "N_total (N_D+ + N_D-)", 800*scale, 200*scale, 2400*scale)  # N_total = N_D+ + N_D-
...
N_total_Ds = RooRealVar("N_total_Ds", "N_total (N_Ds+ + N_Ds-)", 3000*scale, 1000*scale,9000*scale)  # N_total = N_D+ + N_D-
...
#Nbkg_D_plus = ROOT.RooRealVar("Nbkg_D_plus", "Number of background events for D+", 6000*scale, 2000*scale, 10000*scale)
#Nbkg_D_minus = ROOT.RooRealVar("Nbkg_D_minus", "Number of background events for D-", 6000*scale, 2000*scale, 10000*scale)
Nbkg_total = ROOT.RooRealVar("Nbkg_total", "Number of background events for D+", 12000*scale, 6000*scale,15000*scale)
...
x_bkg1_tau = ROOT.RooRealVar("x_bkg1_tau", "c0",-1.5, -20, 5)
...
N_total_Ds_value = N_total_Ds.getVal()
N_total_Ds_error = N_total_Ds.getError()

print(f"N_total = {N_total_value:.0f} ± {N_total_error:.3f}")
print(f"N_total_Ds = {N_total_Ds_value:.0f} ± {N_total_Ds_error:.3f}")

def Linearity_test(sim_model, cat, N_Input=100, N_gen=500, N_total=N_total, N_Input_Ds=1000):
  N_total.setVal(N_Input)
  N_total_Ds.setVal(N_Input_Ds)
  ...
  toyMC_canvas.SaveAs(f"2d/2d_toy_Acp_{tree_name}_N_total_Dp_{N_Input:.2f}_Ds_{N_Input_Ds:.2f}.png")

  Nsig_mean = Mean.getVal()
  Nsig_mean_error = Mean.getError()

  toyMC_frame_N_total_Ds_pull = ToyMC_all.plotPull(N_total_Ds, Bins(20))
  pullMean_Ds = ROOT.RooRealVar("pullMean_Ds","",0,-10,10)
  pullSigma_Ds = ROOT.RooRealVar("pullSigma_Ds","",1,0.1,5)
  pullMean_Ds.setPlotLabel("#mu")
  pullSigma_Ds.setPlotLabel("#sigma")

  pullGauss_Ds = ROOT.RooGaussian("pullGauss_Ds", "", toyMC_frame_N_total_Ds_pull.getPlotVar() , pullMean_Ds, pullSigma_Ds)
  r_pull_Ds = pullGauss_Ds.fitTo(ToyMC_all.fitParDataSet(),NumCPU=6,PrintLevel=-1)

  pullGauss_Ds.plotOn(toyMC_frame_N_total_Ds_pull)
  pullGauss_Ds.paramOn(toyMC_frame_N_total_Ds_pull, ROOT.RooFit.Layout(0.60, 0.80, 0.9), ROOT.RooFit.Format("NE",ROOT.RooFit.AutoPrecision(1)))
  toyMC_Ds_canvas = ROOT.TCanvas("toyMC_Ds_canvas", "D+ fit", 1600, 600)
  toyMC_Ds_canvas.Divide(2,1)
  toyMC_Ds_canvas.cd(1)
  toyMC_frame_N_total_Ds_pull.Draw("PE")

  toyMC_frame_N_total_Ds = ToyMC_all.plotParam(N_total_Ds, Bins(20))
  Mean_Ds = ROOT.RooRealVar("Mean_Ds","",N_Input_Ds, 0.5*N_Input_Ds, 1.5*N_Input_Ds)
  Sigma_Ds = ROOT.RooRealVar("Sigma_Ds","",N_Input_Ds**0.5, N_Input_Ds**0.5/4, N_Input_Ds*0.8)
  Mean_Ds.setPlotLabel("#mu")
  Sigma_Ds.setPlotLabel("#sigma")

  Gauss_Ds = ROOT.RooGaussian("Gauss_Ds", "", toyMC_frame_N_total_Ds.getPlotVar() , Mean_Ds, Sigma_Ds)
  r_N_total_pull_Ds = Gauss_Ds.fitTo(ToyMC_all.fitParDataSet(),NumCPU=6,PrintLevel=-1)

  Gauss_Ds.plotOn(toyMC_frame_N_total_Ds)
  Gauss_Ds.paramOn(toyMC_frame_N_total_Ds, ROOT.RooFit.Layout(0.60, 0.80, 0.9), ROOT.RooFit.Format("NE",ROOT.RooFit.AutoPrecision(1)))
  toyMC_Ds_canvas.cd(2)
  toyMC_frame_N_total_Ds.Draw("PE")
  toyMC_Ds_canvas.SaveAs(f"2d/2d_toy_Acp_{tree_name}_N_total_Ds_Dp_{N_Input:.2f}_Ds_{N_Input_Ds:.2f}.png")

  Nsig_mean_Ds = Mean_Ds.getVal()
  Nsig_mean_error_Ds = Mean_Ds.getError()

  return Nsig_mean, Nsig_mean_error, Nsig_mean_Ds, Nsig_mean_error_Ds


expected_values = np.linspace(N_total_value-N_total_error*3, N_total_value+N_total_error*3, 7)
N_input = list()
N_predict = list()
Fit_error = list()

expected_values_Ds = np.linspace(N_total_Ds_value-N_total_Ds_error*3, N_total_Ds_value+N_total_Ds_error*3, 7)
N_input_Ds = list()
N_predict_Ds = list()
Fit_error_Ds = list()
for i in expected_values:
    for j in expected_values_Ds:
        N_input.append(i)
        N_input_Ds.append(j)

        # Perform the linearity test
        N_fit, temp_fit_error, N_fit_Ds, temp_fit_error_Ds = Linearity_test(sim_model, cat, N_Input=i, N_gen=10, N_Input_Ds=j)
        
        # Append results
        N_predict.append(N_fit)
        Fit_error.append(temp_fit_error)
        N_predict_Ds.append(N_fit_Ds)
        Fit_error_Ds.append(temp_fit_error_Ds)

        print(f'Prediction: ({N_fit}, {N_fit_Ds}) , Input: {i}, Input_Ds: {j}, Error: ({temp_fit_error}, {temp_fit_error_Ds})')
...
# Define the 2D model function
def two_dimensional_model(x, y, c0, c1, c2, c3):
    return (c0 + x * c1) * (c2 + y * c3)

# Define the residuals function
def residuals(c0, c1, c2, c3):
    res = []
    for x, y, pred, err in zip(N_input, N_input_Ds, N_predict, Fit_error):
        model_value = two_dimensional_model(x, y, c0, c1, c2, c3)
        res.append((pred - model_value) / err)
    return np.array(res)

# Define the least-squares cost function
def cost_function(c0, c1, c2, c3):
    return np.sum(residuals(c0, c1, c2, c3) ** 2)

# Initialize Minuit with the cost function and initial guesses
m = Minuit(cost_function, c0=0, c1=0, c2=0, c3=0)

m.migrad()  # Minimize the cost function
m.hesse()   # Compute uncertainties on the parameters

# Generate a mesh grid for visualization
x_range = np.linspace(min(N_input) * 0.97, max(N_input) * 1.03, 101)
y_range = np.linspace(min(N_input_Ds) * 0.97, max(N_input_Ds) * 1.03, 101)
X, Y = np.meshgrid(x_range, y_range)
Z = two_dimensional_model(X, Y, *m.values)

# Plot the results
plt.figure(figsize=(8, 6))
sc = plt.scatter(N_input, N_predict, c=N_input_Ds, cmap='viridis', label='Data')
plt.colorbar(sc, label="N_input_Ds")

# Add 2D fit plot
plt.contourf(X, Y, Z, alpha=0.5, cmap='coolwarm')
plt.xlabel("N_input")
plt.ylabel("N_predict")

# Display legend with fit information
fit_info = [
    f"$\\chi^2$/$n_\\mathrm{{dof}}$ = {m.fval:.1f} / {len(N_input) - m.nfit} = {m.fval / (len(N_input) - m.nfit):.1f}",
]
for p, v, e in zip(m.parameters, m.values, m.errors):
    if p == 'c0':
        p = r'$c_0$'
...
plt.legend(title="\n".join(fit_info), frameon=False, fontsize=10)

plt.tight_layout()
plt.savefig(f"2d/2D_Linearity_{tree_name}_fit.png")
plt.show()
