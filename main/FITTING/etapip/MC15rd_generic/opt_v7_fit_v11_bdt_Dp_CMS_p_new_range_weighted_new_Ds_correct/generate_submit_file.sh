...
#    for value in $(seq 0.65 0.01 0.90); do
#        echo "$script all $value" >> job_list.txt
#    done
#done
#for script in etapip_pipipi_offset_CB.py; do
#    #for value in $(seq 0.51 0.01 0.90); do
#    for value in $(seq 0.78 0.01 0.90); do
#        echo "$script all $value" >> job_list.txt
#    done
#done

for script in etapip_gg_K_offset_CB.py; do
    for value in $(seq 0.75 0.01 0.99); do
        echo "$script all $value" >> job_list.txt
    done
done
for script in etapip_pipipi_K_offset_CB.py; do
    for value in $(seq 0.61 0.01 0.99); do
        echo "$script all $value" >> job_list.txt
    done
done
