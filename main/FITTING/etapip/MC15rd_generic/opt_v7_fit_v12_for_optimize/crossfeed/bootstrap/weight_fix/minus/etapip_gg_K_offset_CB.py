...
suffix = "sumw2fixed"
file_name_Dall = f"/share/storage/jykim/plots/MC15rd/etaKp/gg/generic/crossfeed/bootstrap_sys_MC15rd_etaKp_gg_fit_opt_loose_v7_fitv12_bdt_{args.train}_Dall_CMS_{args.sign}_{BDT_cut}_{suffix}_weighted_Dall"
...
n_crossfeed_total = data.reduce(is_crossfeed_cut_plus).sumEntries() + \
                    data_cc.reduce(is_crossfeed_cut_minus).sumEntries()
# Calculate the relative uncertainty (Poisson Limit)
rel_unc = 1.0 / np.sqrt(n_crossfeed_total) if n_crossfeed_total > 0 else 1.0
print(f"Poisson relative uncertainty for misID: {rel_unc:.5%}")

sys_multiplier = 1 - rel_unc
...
while success_count < n_bootstrap:
    ...
    #sys_multiplier = np.random.normal(1.0, rel_unc)
    weight_formula_plus = f"ds_weight * ({no_crossfeed_cut_plus} ? 1.0 : {sys_multiplier})"
    weight_formula_minus = f"ds_weight * ({no_crossfeed_cut_minus} ? 1.0 : {sys_multiplier})"
