...
parser = argparse.ArgumentParser(
    description=(
        "Plot the sign-averaged MC15rd etapip_gg result using existing "
        "sign-plus/sign-minus RooFitResult files. No refit is performed. "
        "Histograms and lower-pad asymmetry errors use ds_weight."
    )
)
...
parser.add_argument(
    "--suffix",
    default="sumw2fixed",
    help="Suffix used in the original etapip_gg_offset_CB fit-result filenames, before '_weighted.root'",
)
parser.add_argument("--output", default="", help="Output PDF path. If empty, use the default /share/storage/... path")
args = parser.parse_args()

# -----------------------------------------------------------------------------
# Configuration: keep the paths and names consistent with etapip_gg_offset_CB.py.
# -----------------------------------------------------------------------------
BDT_cut = str(args.bdt)
suffix = args.suffix
base_plot_dir = "/share/storage/jykim/plots/MC15rd/etapip/gg/generic"
base_fit_dir = f"{base_plot_dir}/fitresult"

fitresult_name_plus = (
    f"{base_fit_dir}/MC15rd_etapip_gg_fit_opt_loose_v7_fitv12_bdt_"
...
if args.output:
    ...
    file_name_out = (
        f"{base_plot_dir}/MC15rd_etapip_gg_fit_opt_loose_v7_fitv12_bdt_"
        f"{args.train}_Dall_CMS_signAvg_{BDT_cut}_{suffix}_weighted.pdf"
    )
...
# Input ntuples: same logic as etapip_gg_offset_CB.py.
...
ds_weight = ROOT.RooRealVar(weight_var, weight_var, 0, 1000)
...
# -----------------------------------------------------------------------------
# Data import: four weighted samples are built explicitly, then the lower-pad
# asymmetry is averaged over sign plus/minus.
# -----------------------------------------------------------------------------
_weight_columns = []

def get_sign_cut(sign_name):
    if sign_name == "plus":
        return "Dp_CMS_cosTheta>0"
    if sign_name == "minus":
        return "Dp_CMS_cosTheta<0"
    raise ValueError(sign_name)


def get_charge_sign_cut(sign_name, charge):
    return f"{charge_var}=={charge} && rank_Dp_chiProb==1 && {get_sign_cut(sign_name)}"


def make_dataset(name, sign_name, charge):
    cut = get_charge_sign_cut(sign_name, charge)
    print(f"{name}: {cut}")
    before = ROOT.RooDataSet(
        f"{name}_before",
        f"{name} before weighting",
        varset,
        ROOT.RooFit.Import(mychain),
        ROOT.RooFit.Cut(cut),
    )
    w_scaled_name = f"w_scaled_{name}"
    w_scaled = ROOT.RooFormulaVar(
        w_scaled_name,
        "Scaled Weight",
        f"1.0*{weight_var}",
        ROOT.RooArgList(ds_weight),
    )
    before.addColumn(w_scaled)
    data = ROOT.RooDataSet(name, name, before, before.get(), "", w_scaled_name)
    # Keep the formula alive for safety in PyROOT.
    _weight_columns.append(w_scaled)
    print(f"  unweighted entries = {before.sumEntries()}")
    print(f"  weighted entries   = {data.sumEntries()}")
    return data
...
f_sig, result_object_sig = open_fit_result(
    "/share/storage/jykim/plots/MC15rd/etapip/gg/"
    "MC15re_6M_etapip_gg_Dp_M_v12_CB_conv_result_extended_train_Dp_CMS_p.0.80.root"
)
fit_args_sig = result_object_sig.floatParsFinal()

f_sig_Ds, result_object_sig_Ds = open_fit_result(
    "/share/storage/jykim/plots/MC15rd/etapip/gg/"
    "MC15rd_6M_etapip_gg_Dp_M_v12_CB_conv_result_extended_train_Dp_CMS_p_Ds.0.80.weighted.root"
)
fit_args_sig_Ds = result_object_sig_Ds.floatParsFinal()

f_rhopeta, result_object_rhopeta = open_fit_result(
    "/share/storage/jykim/plots/MC15rd/etapip/gg/"
    "MC15rd_etapip_gg_Dp_M_fit_v12_novo_result_rhopeta_from_Dsp_peak_Dp_CMS_p_0.80.root"
)
...
# -----------------------------------------------------------------------------
# Model definition: same PDF structure as etapip_gg_offset_CB.py, but tagged so
...
def build_model(tag):
    N_total = RooRealVar(f"N_total_{tag}", "N_total", 110000 * scale, 0.0, 1.0e7)
    Acp = RooRealVar(f"Acp_{tag}", "Acp", 0.0, -1.0, 1.0)
    ...
    N_total_Ds = RooRealVar(f"N_total_Ds_{tag}", "N_total_Ds", 210000 * scale, 0.0, 1.0e7)
    Acp_Ds = RooRealVar(f"Acp_Ds_{tag}", "Acp_Ds", 0.0, -1.0, 1.0)
    ...
    Nbkg_total = RooRealVar(f"Nbkg_total_{tag}", "Nbkg_total", 300000 * scale, 0.0, 2.0e7)
    Acp_bkg = RooRealVar(f"Acp_bkg_{tag}", "Acp_bkg", 0.0, -0.5, 0.5)
    ...
    novo_mean = ROOT.RooRealVar(f"novo_mean_{tag}", "novo_mean", 1.73, 1.71001, 1.745)
    ...
    bkg_frac = ROOT.RooRealVar(f"bkg_frac_{tag}", "bkg_frac", 0.5, 0.01, 1)
...
# Parameters that were intentionally fixed in etapip_gg_offset_CB.py.
...
# Histogram helpers.
#
# For weighted MC, the bin content is W=sum(w), and the TH1 bin error is
# sqrt(V)=sqrt(sum(w^2)). The asymmetry error is propagated from W_+ and W_-:
#   A = (W_+ - W_-)/(W_+ + W_-)
#   Var(A) = 4*(W_-^2*V_+ + W_+^2*V_-)/(W_+ + W_-)^4.
# -----------------------------------------------------------------------------
def hist_from_chain(name, sign_name, charge, nbins):
    old = ROOT.gROOT.FindObject(name)
    if old:
        old.Delete()

    # Keep the histogram in the current ROOT directory while TChain.Draw fills it.
    # SetDirectory(0) is called after Draw so that the histogram survives later cd() calls.
    h = ROOT.TH1D(name, name, nbins, fit_range[0], fit_range[1])
    h.Sumw2()

    cut = get_charge_sign_cut(sign_name, charge)
    weighted_cut = f"({cut})*{weight_var}"
    draw_expr = f"{fit_variable}>>{name}"
    print(f"{name}: {draw_expr}, weight/cut = {weighted_cut}")
    mychain.Draw(draw_expr, weighted_cut, "goff")
    h.SetDirectory(0)
    return h


h_sp_Dp = hist_from_chain("h_sp_Dp", "plus", 1, args.nbins)
h_sp_Dm = hist_from_chain("h_sp_Dm", "plus", -1, args.nbins)
h_sm_Dp = hist_from_chain("h_sm_Dp", "minus", 1, args.nbins)
h_sm_Dm = hist_from_chain("h_sm_Dm", "minus", -1, args.nbins)
...
def make_asym_hist(hp, hm, name):
    ...
    for ibin in range(1, hp.GetNbinsX() + 1):
        w_p = hp.GetBinContent(ibin)
        w_m = hm.GetBinContent(ibin)
        v_p = hp.GetBinError(ibin) ** 2
        v_m = hm.GetBinError(ibin) ** 2
        total = w_p + w_m
        if total > 0:
            asym = (w_p - w_m) / total
            var_asym = 4.0 * (w_m * w_m * v_p + w_p * w_p * v_m) / (total ** 4)
...
hA_avg.SetMinimum(-0.1)
hA_avg.SetMaximum(0.1)
