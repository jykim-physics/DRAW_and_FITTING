...
parser = argparse.ArgumentParser(
    description=(
        "Plot combined sign-plus/sign-minus result for MC15rd etapip_pipipi using existing "
        "RooFitResult files. No refit is performed. Histograms and lower-pad "
        "asymmetry are filled with ds_weight by default."
    )
)
...
parser.add_argument(
    "--suffix",
    default="sumw2fixed",
    help="Suffix used in the original etapip_pipipi offset fit-result filenames",
)
parser.add_argument("--weight-var", default="ds_weight", help="Weight branch used for MC histograms and asymmetry errors")
parser.add_argument("--output", default="", help="Output PDF path. If empty, use the default /share/storage/... path")
parser.add_argument(
    "--blind",
    action="store_true",
    help="Optionally hide the lower-pad asymmetry in 1.81 < M < 2.02 GeV/c^2",
)
args = parser.parse_args()

# -----------------------------------------------------------------------------
# Configuration: keep the paths and names consistent with etapip_pipipi_offset_CB.py.
# The plus/minus files are assumed to have been produced by running the original
# offset script with --sign plus and --sign minus.
# -----------------------------------------------------------------------------
BDT_cut = str(args.bdt)
suffix = args.suffix
weight_var = args.weight_var
base_plot_dir = "/share/storage/jykim/plots/MC15rd/etapip/pipipi/generic"
base_fit_dir = f"{base_plot_dir}/fitresult"

fitresult_name_plus = (
    f"{base_fit_dir}/MC15rd_etapip_pipipi_fit_opt_loose_v7_fitv12_bdt_"
...
if args.output:
    ...
    file_name_out = (
        f"{base_plot_dir}/MC15rd_etapip_pipipi_fit_opt_loose_v7_fitv12_bdt_"
...
# Input ntuples: same logic as etapip_pipipi_offset_CB.py.
...
if not file_list:
    raise RuntimeError(f"No input ROOT files found for BDT={BDT_cut} under {base_path}")
...
if mychain.GetEntries() == 0:
    raise RuntimeError(f"Input chain '{tree_name}' has zero entries")

if not mychain.GetListOfBranches().FindObject(weight_var):
    raise RuntimeError(f"Weight branch '{weight_var}' was not found in the input chain")
...
# -----------------------------------------------------------------------------
# Event selections for the four weighted histograms.  The histograms are filled
# directly from the TChain with weight_var, so the TH1 bin errors are sqrt(sum w^2).
# -----------------------------------------------------------------------------
def make_cut(sign_name, charge):
    if sign_name == "plus":
    ...
    return f"{charge_var}=={charge} && rank_Dp_chiProb==1 && {sign_cut}"


cuts = {
    "sp_Dp": make_cut("plus", 1),
    "sp_Dm": make_cut("plus", -1),
    "sm_Dp": make_cut("minus", 1),
    "sm_Dm": make_cut("minus", -1),
}

print("Entries before weighting:")
for key, cut in cuts.items():
    print(f"  {key}: {mychain.GetEntries(cut)}")
...
def build_model(tag):
    N_total = RooRealVar(f"N_total_{tag}", "N_total", 60000 * scale, 30000 * scale, 2400000 * scale)
    ...
    N_total_Ds = RooRealVar(
        f"N_total_Ds_{tag}", "N_total_Ds", 140000 * scale, 50000 * scale, 1400000 * scale
    )
    ...
    Nbkg_total = RooRealVar(
        f"Nbkg_total_{tag}", "Nbkg_total", 250000 * scale, 20000 * scale, 2500000 * scale
    )
...
# -----------------------------------------------------------------------------
# Histogram helpers.  Weighted asymmetry error propagation:
#   A = (P - M)/(P + M), Var(P)=sum w_P^2, Var(M)=sum w_M^2.
#   sigma_A^2 = (2M/S^2)^2 Var(P) + (2P/S^2)^2 Var(M).
# -----------------------------------------------------------------------------
def hist_from_chain(name, cut, nbins):
    h = ROOT.TH1D(name, "", nbins, fit_range[0], fit_range[1])
    h.Sumw2()
    weighted_cut = f"({weight_var})*({cut})"
    n_drawn = mychain.Project(name, fit_variable, weighted_cut)
    h.SetDirectory(0)
    print(
        f"{name}: selected={n_drawn}, sumw={h.Integral():.6g}, "
        f"sumw2^0.5={math.sqrt(sum(h.GetBinError(i)**2 for i in range(1, h.GetNbinsX()+1))):.6g}"
    )
    return h


h_sp_Dp = hist_from_chain("h_sp_Dp", cuts["sp_Dp"], args.nbins)
h_sp_Dm = hist_from_chain("h_sp_Dm", cuts["sp_Dm"], args.nbins)
h_sm_Dp = hist_from_chain("h_sm_Dp", cuts["sm_Dp"], args.nbins)
h_sm_Dm = hist_from_chain("h_sm_Dm", cuts["sm_Dm"], args.nbins)
...
def make_asym_hist(hp, hm, name):
    ...
    for ibin in range(1, hp.GetNbinsX() + 1):
        ...
        if abs(total) > 0:
            asym = (n_p - n_m) / total
            err = 2.0 * math.sqrt(max(0.0, n_m * n_m * var_p + n_p * n_p * var_m)) / (total * total)
...
# Optional blinding of the lower-pad asymmetry.  Disabled by default for MC plots.
if args.blind:
    blind_min = 1.81
    blind_max = 2.02
