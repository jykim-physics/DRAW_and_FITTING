...
#range_min, range_max = -0.03, 0.03
range_min, range_max = -0.05, 0.05
bins = 50
...
axes[0].text(0.05, 0.95, f"Mean = {mean1*100:.4f}%\nStd = {std1*100:.4f}%", transform=axes[0].transAxes,
...
axes[1].text(0.05, 0.95, f"Mean = {mean2*100:.4f}%\nStd = {std2*100:.4f}%", transform=axes[1].transAxes,
             verticalalignment='top', bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))

plt.tight_layout()
plt.savefig(f"plots/delta_acp_{mode}.png")
plt.show()

print(f"cos minus: mean = {mean1:.6f}, std = {std1:.6f}")
print(f"cos plus: mean = {mean2:.6f}, std = {std2:.6f}")

# Combined statistics
combined_mean = (mean1 + mean2) / 2
combined_std = np.sqrt(std1**2 + std2**2)

print(f"\nCombined mean: {combined_mean:.6f}")
print(f"Combined error (std): {combined_std:.6f}")
