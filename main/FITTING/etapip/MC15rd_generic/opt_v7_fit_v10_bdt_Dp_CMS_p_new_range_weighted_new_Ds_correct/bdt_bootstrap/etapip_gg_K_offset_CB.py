...
print(f"Num_total of data = {Num_total}")
...
print(f"Num_total of data_cc = {Num_total_cc}")

N_total = RooRealVar("N_total", "N_total (N_D+ + N_D-)", 2500*scale*N_scale, 0*scale*N_scale, 12000*scale*N_scale)  # N_total = N_D+ + N_D-
...
Nbkg_total = ROOT.RooRealVar("Nbkg_total", "Number of background events for D+", 40000*scale*N_scale, 0*scale*N_scale,400000*scale*N_scale)
...
while success_count < n_bootstrap:
    ...
    cut_expr_nominal = "BDT > 0.91"
    eff_nominal = 0.0455
    eff_Ds_nominal = 0.038620
...
with uproot.recreate(f"bootstrap_results_etapip_gg_K_{args.sign}_converge.root") as f:
    f["tree"] = ak_array
