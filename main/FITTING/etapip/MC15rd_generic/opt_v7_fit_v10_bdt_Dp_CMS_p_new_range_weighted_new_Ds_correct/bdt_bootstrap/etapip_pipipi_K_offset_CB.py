...
N_total = RooRealVar("N_total", "N_total (N_D+ + N_D-)", 1200*scale*N_scale, 0*scale*N_scale, 10000*scale*N_scale)  # N_total = N_D+ + N_D-
...
Nbkg_total = ROOT.RooRealVar("Nbkg_total", "Number of background events for D+", 3000*scale*N_scale, 0*scale*N_scale,20000*scale*N_scale)
...
while success_count < n_bootstrap:
    ...
    eff_nominal = 0.038100
    eff_Ds_nominal = 0.027621
...
with uproot.recreate(f"bootstrap_results_etapip_pipipi_K_{args.sign}_converge.root") as f:
    f["tree"] = ak_array
