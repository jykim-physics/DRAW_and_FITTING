...
N_total = RooRealVar("N_total", "N_total (N_D+ + N_D-)", 50000*scale*N_scale, 10000*scale*N_scale, 250000*scale*N_scale)  # N_total = N_D+ + N_D-
...
N_total_Ds = RooRealVar("N_total_Ds", "N_total (N_Ds+ + N_Ds-)", 150000*scale*N_scale, 20000*scale*N_scale, 500000*scale*N_scale)  # N_total = N_D+ + N_D-
...
Nbkg_total = ROOT.RooRealVar("Nbkg_total", "Number of background events for D+", 600000*scale*N_scale, 20000*scale*N_scale,1600000*scale*N_scale)
...
while success_count < n_bootstrap:
    ...
    eff_nominal = 0.063126
    eff_Ds_nominal = 0.054430
...
with uproot.recreate(f"bootstrap_results_ref_etapip_gg_{args.sign}_converge.root") as f:
    f["tree"] = ak_array
