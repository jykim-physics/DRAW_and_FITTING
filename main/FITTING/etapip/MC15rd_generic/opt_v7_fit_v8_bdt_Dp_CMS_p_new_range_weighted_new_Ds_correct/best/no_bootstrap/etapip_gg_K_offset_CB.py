...
file_name_Dall = f"/share/storage/jykim/plots/MC15rd/etaKp/gg/generic/no_bootstrap/sys_MC15rd_etaKp_gg_loose_v7_fitv8_bdt_{args.train}_{args.sign}_{BDT_cut}_{suffix}_weighted_Dall"
fitresult_name = f"/share/storage/jykim/plots/MC15rd/etaKp/gg/generic/fitresult/no_bootstrap/sys_MC15rd_etaKp_gg_loose_v7_fitv8_bdt_{args.train}_{args.sign}_{BDT_cut}_{suffix}_weighted.root"
fitresult_text = f"/share/storage/jykim/plots/MC15rd/etaKp/gg/generic/fitresult/no_bootstrap/sys_MC15rd_etaKp_gg_loose_v7_fitv8_bdt_{args.train}_{args.sign}_{BDT_cut}_{suffix}_weighted.txt"
...
'''
f = ROOT.TFile.Open(f"/share/storage/jykim/plots/MC15rd/etaKp/gg/generic/fitresult/MC15rd_etaKp_gg_fit_opt_loose_v7_fitv8_bdt_train_Dp_CMS_p_{args.sign}_0.91_new_Ds_correct_weighted.root")
...
N_total_var = RooRealVar("N_total_var", "N_total (N_D+ + N_D-)", 2500*scale*N_scale, 0*scale*N_scale, 6000*scale*N_scale)  # N_total = N_D+ + N_D-
...
N_total_Ds_var = RooRealVar("N_total_Ds_var", "N_total (N_Ds+ + N_Ds-)", 16000*scale*N_scale, 0*scale*N_scale,50000*scale*N_scale)  # N_total = N_D+ + N_D-
...
Nbkg_total_var = ROOT.RooRealVar("Nbkg_total_var", "Number of background events for D+", 40000*scale*N_scale, 0*scale*N_scale,200000*scale*N_scale)
...
f_sig = ROOT.TFile.Open(f"/share/storage/jykim/plots/MC15rd/etaKp/gg/MC15re_6M_etapip_gg_K_Dp_M_opt_v7_CB_conv_result_extended_train_Dp_CMS_p.0.91_new_Ds_correct.root")
...
f_sig_Ds = ROOT.TFile.Open(f"/share/storage/jykim/plots/MC15rd/etaKp/gg/MC15rd_6M_etapip_gg_K_Dp_M_opt_v7_CB_conv_result_extended_train_Dp_CMS_p_Ds.0.91.weighted.new_Ds_correct.root")
...
def var_fit_extract_Acp(data_Dp, data_Dm, n_bootstrap):
  ...
  sigma_gaussian_var = ROOT.RooRealVar("sigma_gaussian_var", "sigma of Gaussian", sample_param( fit_sigma_gaussian.getVal(),fit_sigma_gaussian.getError()))
  ...
  Ds_sigma_gaussian_var = ROOT.RooRealVar("Ds_sigma_gaussian_var", "sigma of Gaussian", sample_param( fit_Ds_sigma_gaussian.getVal(), fit_Ds_sigma_gaussian.getError()))

  scaled_sigma_gaussian_var = RooFormulaVar("scaled_sigma_gaussian_var","sigma_gaussian_var * scale_factor_var",RooArgList(sigma_gaussian_var, scale_factor_var))
  scaled_Ds_sigma_gaussian_var = RooFormulaVar("Ds_scaled_sigma_gaussian_var","Ds_sigma_gaussian_var * scale_factor_var",RooArgList(Ds_sigma_gaussian_var, scale_factor_var))
...
while success_count < n_bootstrap:
    ...
    fit_outputs_after = var_fit_extract_Acp(data, data_cc, iteration)
    ...
    if sm_a == 0 and sh_a == 0:
        ...
        success_count += 1
        print(f"[{success_count}/{n_bootstrap}] successful bootstrap.")
    else:
        print(f"[{iteration}] Fit did not converge properly. Retrying... (sm_a={sm_a}, sh_a={sh_a})")


ak_array = ak.Array(results)
with uproot.recreate(f"no_bootstrap_acp_results_etapip_gg_K_{args.sign}_converge_250710.root") as f:
    f["acp_bootstrap"] = ak_array

# Final results
acp_array = np.array(results["acp_after"])
mean_acp = np.mean(acp_array)
std_acp = np.std(acp_array)

acp_Ds_array = np.array(results["acp_Ds_after"])
mean_acp_Ds = np.mean(acp_Ds_array)
std_acp_Ds = np.std(acp_Ds_array)
print(f"Bootstrap results:")
print(f"  D+ acp    = {mean_acp:.8f} pm {std_acp:.8f}")
print(f"  Ds+ acp    = {mean_acp_Ds:.8f} pm {std_acp_Ds:.8f}")
