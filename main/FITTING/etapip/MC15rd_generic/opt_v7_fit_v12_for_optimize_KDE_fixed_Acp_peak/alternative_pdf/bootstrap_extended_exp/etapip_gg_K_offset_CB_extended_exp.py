...
suffix = "KDE_fixed_Acp_peak_alternative_pdf_extended_exp"
...
f = ROOT.TFile.Open(f"/share/storage/jykim/plots/MC15rd/etaKp/gg/generic/fitresult/MC15rd_etaKp_gg_fit_opt_loose_v7_fitv12_1_bdt_train_Dp_CMS_p_{args.sign}_0.86_KDE_fixed_Acp_peak_weighted.root")
...
x_bkg1_tau_var2 = ROOT.RooRealVar("x_bkg1_tau_var2", "c0",-5, -20, 10)
...
def var_fit_extract_Acp(data_Dp, data_Dm, n_bootstrap):
  ...
  #x_bkg1_c1_var.setVal(0.1)
  #x_bkg1_c2_var.setVal(0.1)
  ...
  model_bkg_var = ROOT.RooGenericPdf("model_bkg_var","exp(@1*@0 + @2*@0*@0)",ROOT.RooArgList(x, x_bkg1_tau_var, x_bkg1_tau_var2))


  model_D_plus_var= ROOT.RooAddPdf("model_D_plus_var", "D+ model",
                                ROOT.RooArgList(sig_model_var, Ds_model_var, model_bkg_var, kde_model_var),
...
while success_count < n_bootstrap:
    ...
    rng = np.random.default_rng(seed)
    print(f"[Bootstrap] iteration = {iteration}, seed = {seed}")

    for _ in range(n_entries_Dp):
        #idx = rng.randint(0, data.numEntries() - 1)
        idx = rng.integers(0, data.numEntries())
        data_boot_Dp.add(data.get(int(idx)), data.weight())

    for _ in range(n_entries_Dm):
        #idx = rng.randint(0, data_cc.numEntries() - 1)
        idx = rng.integers(0, data_cc.numEntries())
        data_boot_Dm.add(data_cc.get(int(idx)), data_cc.weight())
