...
file_name_Dp = f"/share/storage/jykim/plots/MC15rd/etaKp/pipipi/generic/MC15rd_etaKp_pipipi_fit_opt_loose_v7_fitv14_bdt_{args.train}_Dp_CMS_{args.sign}_{BDT_cut}_{suffix}_weighted.png"
file_name_Dm = f"/share/storage/jykim/plots/MC15rd/etaKp/pipipi/generic/MC15rd_etaKp_pipipi_fit_opt_loose_v7_fitv14_bdt_{args.train}_Dm_CMS_{args.sign}_{BDT_cut}_{suffix}_weighted.png"
file_name_Dall = f"/share/storage/jykim/plots/MC15rd/etaKp/pipipi/generic/MC15rd_etaKp_pipipi_fit_opt_loose_v7_fitv14_bdt_{args.train}_Dall_CMS_{args.sign}_{BDT_cut}_{suffix}_weighted.pdf"
fitresult_name = f"/share/storage/jykim/plots/MC15rd/etaKp/pipipi/generic/fitresult/MC15rd_etaKp_pipipi_fit_opt_loose_v7_fitv14_bdt_{args.train}_{args.sign}_{BDT_cut}_{suffix}_weighted.root"
fitresult_text = f"/share/storage/jykim/plots/MC15rd/etaKp/pipipi/generic/fitresult/MC15rd_etaKp_pipipi_fit_opt_loose_v7_fitv14_bdt_{args.train}_{args.sign}_{BDT_cut}_{suffix}_weighted.txt"
...
N_total = RooRealVar("N_total", "N_total (N_D+ + N_D-)", 1200*scale*N_scale, 0*scale*N_scale, 50000*scale*N_scale)  # N_total = N_D+ + N_D-
...
N_total_Ds = RooRealVar("N_total_Ds", "N_total (N_Ds+ + N_Ds-)", 4000*scale*N_scale, 0*scale*N_scale,100000*scale*N_scale)  # N_total = N_D+ + N_D-
...
N_peak_bkg_total = ROOT.RooRealVar("N_peak_bkg_total", "Number of background events",N_peak_bkg_total_RooRealVar.getVal())
...
x_bkg1_tau1 = ROOT.RooRealVar("x_bkg1_tau1", "c0",-5, -20, 5)
x_bkg1_tau2 = ROOT.RooRealVar("x_bkg1_tau2", "c0",-1, -10, 10)

Acp_mean = ROOT.RooRealVar("Acp_mean", "mean", 0, -1, 1)
Acp_scale_factor = ROOT.RooRealVar("Acp_scale_factor", "scale", 0, -1, 1)
Acp_Ds_mean = ROOT.RooRealVar("Acp_Ds_mean", "mean", 0, -1, 1)
Acp_x_bkg1_tau1 = ROOT.RooRealVar("Acp_x_bkg1_tau1", "c0",0.0, -1.0, 1.0)
Acp_x_bkg1_tau2 = ROOT.RooRealVar("Acp_x_bkg1_tau2", "c0",0.0, -1.0, 1.0)

mean_plus = ROOT.RooRealVar("mean_plus", "mean", 1.87, 1.85, 1.89)
scale_factor_plus = ROOT.RooRealVar("scale_factor_plus", "sigma of Gaussian", 1,0,2)
Ds_mean_plus = ROOT.RooRealVar("Ds_mean_plus", "mean", 1.97, 1.95, 1.99)
x_bkg1_tau1_plus = ROOT.RooRealVar("x_bkg1_tau1_plus", "c0", -5 , -20, 5)
x_bkg1_tau2_plus = ROOT.RooRealVar("x_bkg1_tau2_plus", "c0", -1 , -10, 10)

mean_minus = RooFormulaVar("mean_minus","mean_plus * (1 - Acp_mean)/(1 + Acp_mean)", RooArgList(mean_plus, Acp_mean))
scale_factor_minus = RooFormulaVar("scale_factor_minus","scale_factor_plus * (1 - Acp_scale_factor)/(1 + Acp_scale_factor)",RooArgList(scale_factor_plus, Acp_scale_factor))
Ds_mean_minus = RooFormulaVar("Ds_mean_minus","Ds_mean_plus * (1 - Acp_Ds_mean)/(1 + Acp_Ds_mean)", RooArgList(Ds_mean_plus, Acp_Ds_mean))
x_bkg1_tau1_minus = RooFormulaVar("x_bkg1_tau1_minus","x_bkg1_tau1_plus * (1 - Acp_x_bkg1_tau1)/(1 + Acp_x_bkg1_tau1)", RooArgList(x_bkg1_tau1_plus,Acp_x_bkg1_tau1))
x_bkg1_tau2_minus = RooFormulaVar("x_bkg1_tau2_minus","x_bkg1_tau2_plus * (1 - Acp_x_bkg1_tau2)/(1 + Acp_x_bkg1_tau2)", RooArgList(x_bkg1_tau2_plus,Acp_x_bkg1_tau2))
...
if args.float_var == "mean":
    ...
    model_bkg = ROOT.RooGenericPdf("model_bkg","exp(@1*(@0-1.919) + @2*(@0-1.919)*(@0-1.919))",ROOT.RooArgList(x, x_bkg1_tau1, x_bkg1_tau2))

    model_D_plus = ROOT.RooAddPdf("model_D_plus", "D+ model",
                                  ROOT.RooArgList(sig_model_plus, Ds_model, model_bkg, kde_model),
                                  ROOT.RooArgList(Nsig_D_plus, Nsig_Ds_plus,  Nbkg_D_plus, N_peak_bkg_D_plus))
    model_D_minus = ROOT.RooAddPdf("model_D_minus", "D- model",
                                  ROOT.RooArgList(sig_model_minus, Ds_model,  model_bkg, kde_model),
                                  ROOT.RooArgList(Nsig_D_minus, Nsig_Ds_minus,  Nbkg_D_minus, N_peak_bkg_D_minus))
elif args.float_var == "scale_factor":
    CB = ROOT.RooCrystalBall("CB", "CB_left", x, mean, sigmaL,sigmaR, alphaL, nL, alphaR, nR)
    scaled_sigma_gaussian_plus = RooFormulaVar("scaled_sigma_gaussian_plus","sigma_gaussian * scale_factor_plus", RooArgList(sigma_gaussian,          scale_factor_plus))
    scaled_sigma_gaussian_minus = RooFormulaVar("scaled_sigma_gaussian_minus","sigma_gaussian * scale_factor_minus", RooArgList(sigma_gaussian,       scale_factor_minus))
    gaussian_plus = ROOT.RooGaussian("gaussian_plus", "Gaussian PDF", x, mean_gaussian, scaled_sigma_gaussian_plus)
    gaussian_minus = ROOT.RooGaussian("gaussian_minus", "Gaussian PDF", x, mean_gaussian, scaled_sigma_gaussian_minus)
    ...
    Ds_CB = ROOT.RooCrystalBall("Ds_CB", "CB_left", x, Ds_mean, Ds_sigmaL, Ds_sigmaR, Ds_alphaL, Ds_nL, Ds_alphaR, Ds_nR)
    scaled_Ds_sigma_gaussian_plus = RooFormulaVar("Ds_scaled_sigma_gaussian_plus", "Ds_sigma_gaussian * scale_factor_plus",                           RooArgList(Ds_sigma_gaussian, scale_factor_plus))
    scaled_Ds_sigma_gaussian_minus = RooFormulaVar("Ds_scaled_sigma_gaussian_minus", "Ds_sigma_gaussian * scale_factor_minus",                        RooArgList(Ds_sigma_gaussian, scale_factor_minus))
    Ds_gaussian_plus = ROOT.RooGaussian("Ds_gaussian_plus", "Gaussian PDF", x, Ds_mean_gaussian, scaled_Ds_sigma_gaussian_plus)
    Ds_gaussian_minus = ROOT.RooGaussian("Ds_gaussian_minus", "Gaussian PDF", x, Ds_mean_gaussian, scaled_Ds_sigma_gaussian_minus)
    ...
    model_D_plus = ROOT.RooAddPdf("model_D_plus", "D+ model",
                                  ROOT.RooArgList(sig_model_plus, Ds_model_plus, model_bkg, kde_model),
                                  ROOT.RooArgList(Nsig_D_plus, Nsig_Ds_plus,  Nbkg_D_plus, N_peak_bkg_D_plus))
    model_D_minus = ROOT.RooAddPdf("model_D_minus", "D- model",
                                  ROOT.RooArgList(sig_model_minus, Ds_model_minus,  model_bkg, kde_model),
                                  ROOT.RooArgList(Nsig_D_minus, Nsig_Ds_minus,  Nbkg_D_minus, N_peak_bkg_D_minus))
    ...
    model_D_plus = ROOT.RooAddPdf("model_D_plus", "D+ model",
                                  ROOT.RooArgList(sig_model, Ds_model_plus, model_bkg, kde_model),
                                  ROOT.RooArgList(Nsig_D_plus, Nsig_Ds_plus,  Nbkg_D_plus, N_peak_bkg_D_plus))
    model_D_minus = ROOT.RooAddPdf("model_D_minus", "D- model",
                                  ROOT.RooArgList(sig_model, Ds_model_minus,  model_bkg, kde_model),
                                  ROOT.RooArgList(Nsig_D_minus, Nsig_Ds_minus,  Nbkg_D_minus, N_peak_bkg_D_minus))
elif args.float_var == "x_bkg1_tau1":
    CB = ROOT.RooCrystalBall("CB", "CB_left", x, mean, sigmaL,sigmaR, alphaL, nL, alphaR, nR)
    scaled_sigma_gaussian = RooFormulaVar("scaled_sigma_gaussian","sigma_gaussian * scale_factor", RooArgList(sigma_gaussian, scale_factor))
    gaussian = ROOT.RooGaussian("gaussian", "Gaussian PDF", x, mean_gaussian, scaled_sigma_gaussian)
    sig_model = ROOT.RooFFTConvPdf("sig_model", "Convolution of Johnson and Gaussian", x, CB, gaussian)
    ...
    model_bkg_plus = ROOT.RooGenericPdf("model_bkg_plus","exp(@1*(@0-1.919) + @2*(@0-1.919)*(@0-1.919))",ROOT.RooArgList(x, x_bkg1_tau1_plus, x_bkg1_tau2))
    model_bkg_minus = ROOT.RooGenericPdf("model_bkg_minus","exp(@1*(@0-1.919) + @2*(@0-1.919)*(@0-1.919))",ROOT.RooArgList(x, x_bkg1_tau1_minus, x_bkg1_tau2))
    ...
    model_D_minus = ROOT.RooAddPdf("model_D_minus", "D- model",
                                  ...
                                  ROOT.RooArgList(Nsig_D_minus, Nsig_Ds_minus,  Nbkg_D_minus, N_peak_bkg_D_minus))
elif args.float_var == "x_bkg1_tau2":
    CB = ROOT.RooCrystalBall("CB", "CB_left", x, mean, sigmaL,sigmaR, alphaL, nL, alphaR, nR)
    scaled_sigma_gaussian = RooFormulaVar("scaled_sigma_gaussian","sigma_gaussian * scale_factor", RooArgList(sigma_gaussian, scale_factor))
    gaussian = ROOT.RooGaussian("gaussian", "Gaussian PDF", x, mean_gaussian, scaled_sigma_gaussian)
    sig_model = ROOT.RooFFTConvPdf("sig_model", "Convolution of Johnson and Gaussian", x, CB, gaussian)
    ...
    model_bkg_plus = ROOT.RooGenericPdf("model_bkg_plus","exp(@1*(@0-1.919) + @2*(@0-1.919)*(@0-1.919))",ROOT.RooArgList(x, x_bkg1_tau1, x_bkg1_tau2_plus))
    model_bkg_minus = ROOT.RooGenericPdf("model_bkg_minus","exp(@1*(@0-1.919) + @2*(@0-1.919)*(@0-1.919))",ROOT.RooArgList(x, x_bkg1_tau1, x_bkg1_tau2_minus))
...
fit_result = sim_model.fitTo(data_combined, RooFit.Save(), RooFit.Extended(True), RooFit.SumW2Error(True), ROOT.RooFit.NumCPU(8), RooFit.Strategy(1), ROOT.RooFit.Offset(False))
