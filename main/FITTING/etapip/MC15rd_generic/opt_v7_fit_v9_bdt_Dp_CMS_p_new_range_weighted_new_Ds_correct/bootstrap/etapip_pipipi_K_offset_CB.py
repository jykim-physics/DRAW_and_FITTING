...
file_name_Dall = f"/share/storage/jykim/plots/MC15rd/etaKp/pipipi/generic/bootstrap_sys_MC15rd_etaKp_pipipi_loose_v7_fitv9_bdt_{args.train}_{args.sign}_{BDT_cut}_{suffix}_weighted_Dall"
fitresult_name = f"/share/storage/jykim/plots/MC15rd/etaKp/pipipi/generic/fitresult/bootstrap_sys_MC15rd_etaKp_pipipi_loose_v7_fitv9_bdt_{args.train}_{args.sign}_{BDT_cut}_{suffix}_weighted_v2.root"
fitresult_text = f"/share/storage/jykim/plots/MC15rd/etaKp/pipipi/generic/fitresult/bootstrap_sys_MC15rd_etaKp_pipipi_loose_v7_fitv9_bdt_{args.train}_{args.sign}_{BDT_cut}_{suffix}_weighted_v2.txt"
...
N_total_Ds = RooRealVar("N_total_Ds", "N_total (N_Ds+ + N_Ds-)", 8000*scale*N_scale, 0*scale*N_scale,15000*scale*N_scale)  # N_total = N_D+ + N_D-
...
Nbkg_total = ROOT.RooRealVar("Nbkg_total", "Number of background events for D+", 3000*scale*N_scale, 0*scale*N_scale,10000*scale*N_scale)
...
f = ROOT.TFile.Open(f"/share/storage/jykim/plots/MC15rd/etaKp/pipipi/generic/fitresult/MC15rd_etaKp_pipipi_fit_opt_loose_v7_fitv9_bdt_train_Dp_CMS_p_{args.sign}_0.92_new_Ds_correct_weighted_v2.root")
...
def nominal_fit_extract_Acp(data_Dp, data_Dm):
  ...
  Ds_CB = ROOT.RooCrystalBall("Ds_CB", "CB_left", x, Ds_mean, Ds_sigmaLR, Ds_alphaL, Ds_nL, Ds_alphaR, Ds_nR)
...
N_total_var = RooRealVar("N_total_var", "N_total (N_D+ + N_D-)", 1200*scale*N_scale, 0*scale*N_scale, 5000*scale*N_scale)  # N_total = N_D+ + N_D-
...
N_total_Ds_var = RooRealVar("N_total_Ds_var", "N_total (N_Ds+ + N_Ds-)", 8000*scale*N_scale, 0*scale*N_scale,15000*scale*N_scale)  # N_total = N_D+ + N_D-
...
Nbkg_total_var = ROOT.RooRealVar("Nbkg_total_var", "Number of background events for D+", 3000*scale*N_scale, 0*scale*N_scale,10000*scale*N_scale)
...
f_sig = ROOT.TFile.Open(f"/share/storage/jykim/plots/MC15rd/etaKp/pipipi/MC15re_6M_etapip_pipipi_K_Dp_M_opt_v7_DSCBShape_conv_result_extended_train_Dp_CMS_p.0.92_new_Ds_correct_v2.root")
...
f_sig_Ds = ROOT.TFile.Open(f"/share/storage/jykim/plots/MC15rd/etaKp/pipipi/MC15rd_6M_etapip_pipipi_K_Dp_M_opt_v7_DSCBShape_conv_result_extended_train_Dp_CMS_p_Ds.0.92.new_Ds_correct.weighted.root")
...
fit_sigmaLR= fit_args_sig.find("sigmaLR")
...
fit_Ds_sigmaLR = fit_args_sig_Ds.find("sigmaLR")
...
def var_fit_extract_Acp(data_Dp, data_Dm, n_bootstrap):
  sigmaLR_var= ROOT.RooRealVar("sigmaLR_var", "sigmaL_var", sample_param( fit_sigmaLR.getVal(), fit_sigmaLR.getError()))
  ...
  Ds_sigmaLR_var = ROOT.RooRealVar("Ds_sigmaLR_var", "sigma_var", sample_param( fit_Ds_sigmaLR.getVal(), fit_Ds_sigmaLR.getError()))
  ...
  CB_var= ROOT.RooCrystalBall("CB_var", "CB_left", x, mean_var, sigmaLR_var, alphaL_var, nL_var, alphaR_var, nR_var)
  ...
  Ds_CB_var= ROOT.RooCrystalBall("Ds_CB_var", "CB_left", x, Ds_mean_var, Ds_sigmaLR_var, Ds_alphaL_var, Ds_nL_var, Ds_alphaR_var, Ds_nR_var)
...
for i in range(n_bootstrap):
    ...
    fit_outputs_before = var_fit_extract_Acp(data_boot_Dp, data_boot_Dm, i)
...
with uproot.recreate(f"bootstrap_acp_results_etapip_pipipi_K_{args.sign}_converge_DSCBShape_v2.root") as f:
