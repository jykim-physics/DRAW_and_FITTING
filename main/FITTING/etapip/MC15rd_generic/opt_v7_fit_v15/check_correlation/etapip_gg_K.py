...
suffix = "KDE_fixed_2nd_cheby_correlation_study"
...
fit_result = sim_model.fitTo(data_combined, RooFit.Save(True), RooFit.Extended(True), RooFit.SumW2Error(True), ROOT.RooFit.NumCPU(8), RooFit.Strategy(1), ROOT.RooFit.Offset(True))
...
def _get_float_parameter_names(fit_result):
    params = fit_result.floatParsFinal()
    return [params[i].GetName() for i in range(params.getSize())]

def print_correlations_with_target(fit_result, target_name="Acp_peak_bkg", out_txt=None):
    names = _get_float_parameter_names(fit_result)
    if target_name not in names:
        print(f"[WARN] {target_name} is not found in floatParsFinal().")
        print("[INFO] Floating parameters are:", names)
        return []
    ...
    for name in names:
        if name == target_name:
            continue
        corr = fit_result.correlation(target_name, name)
        rows.append((name, corr))
    rows.sort(key=lambda item: abs(item[1]), reverse=True)

    lines = []
    lines.append(f"Correlations with {target_name}")
    lines.append("parameter correlation")
    lines.append("--------------------------------------")
    for name, corr in rows:
        lines.append(f"{name:<25s} {corr:+.4f}")

    print("".join(lines))

    if out_txt is not None:
        with open(out_txt, "w") as f_corr:
            f_corr.writelines(lines)
        print(f"[INFO] Correlation table saved to {out_txt}")

    return rows

def draw_correlation_matrix(fit_result, names, out_prefix, title="Correlation matrix"):
    n = len(names)
    if n == 0:
        print("[WARN] Empty parameter list for correlation matrix.")
        return

    h = ROOT.TH2D("h_corr_matrix", title, n, 0, n, n, 0, n)
    h.SetStats(False)
    h.SetMinimum(-1.0)
    h.SetMaximum(1.0)

    for i, name_i in enumerate(names):
        h.GetXaxis().SetBinLabel(i + 1, name_i)
        h.GetYaxis().SetBinLabel(i + 1, name_i)
        for j, name_j in enumerate(names):
            h.SetBinContent(i + 1, j + 1, fit_result.correlation(name_i, name_j))

    canvas_size = max(900, 55*n + 350)
    c = ROOT.TCanvas("c_corr_matrix", title, canvas_size, canvas_size)
    ...
    h.Draw("COLZ TEXT")
    c.SaveAs(out_prefix + ".pdf")
    c.SaveAs(out_prefix + ".png")
    #print(f"[INFO] Correlation matrix saved to {out_prefix}.pdf/.png")

def draw_correlation_row(fit_result, target_name, names, out_prefix):
    if target_name not in names:
        print(f"[WARN] {target_name} is not found in floatParsFinal().")
        return

    other_names = [name for name in names if name != target_name]
    # Sort by absolute correlation to make the important correlations appear first.
    other_names.sort(key=lambda name: abs(fit_result.correlation(target_name, name)), reverse=True)

    n = len(other_names)
    h = ROOT.TH2D("h_corr_row", f"Correlation with {target_name}", n, 0, n, 1, 0, 1)
    h.SetStats(False)
    h.SetMinimum(-1.0)
    h.SetMaximum(1.0)
    h.GetYaxis().SetBinLabel(1, target_name)

    for i, name in enumerate(other_names):
        h.GetXaxis().SetBinLabel(i + 1, name)
        h.SetBinContent(i + 1, 1, fit_result.correlation(target_name, name))

    c = ROOT.TCanvas("c_corr_row", f"Correlation with {target_name}", max(900, 60*n + 250), 360)
    ...
    c.SaveAs(out_prefix + ".pdf")
    c.SaveAs(out_prefix + ".png")
    #print(f"[INFO] Correlation row saved to {out_prefix}.pdf/.png")

corr_out_prefix = fitresult_name.replace(".root", "")
float_param_names = _get_float_parameter_names(fit_result)

print_correlations_with_target(
    fit_result,
    target_name="Acp_peak_bkg",
    out_txt=corr_out_prefix + "_corr_with_Acp_peak_bkg.txt"
)

# Full floating-parameter correlation matrix.
draw_correlation_matrix(
    fit_result,
    float_param_names,
    corr_out_prefix + "_corr_matrix_all_float",
    title="Correlation matrix: all floating parameters"
)

# Compact one-row heatmap: Acp_peak_bkg vs the other floating parameters.
draw_correlation_row(
    fit_result,
    "Acp_peak_bkg",
    float_param_names,
    corr_out_prefix + "_corr_Acp_peak_bkg"
)
