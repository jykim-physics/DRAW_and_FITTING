...
parser = argparse.ArgumentParser(
    description=(
        "Plot combined sign-plus/sign-minus weighted MC result for etapip_pipipi_K using existing "
        "RooFitResult files. No refit is performed. The lower-pad asymmetry is "
        "0.5*(A_raw(sign plus) + A_raw(sign minus)), with ds_weight-aware errors."
    )
)
...
parser.add_argument(
    ...
    help="Suffix used in the original weighted MC etapip_pipipi_K fit-result filenames",
)
parser.add_argument("--output", default="", help="Output PDF path. If empty, use the default /share/storage/... path")
parser.add_argument(
    "--no-blind",
    action="store_true",
    help="Kept for interface compatibility; weighted MC signAvg plot is not blinded by default.",
)
args = parser.parse_args()

BDT_cut = str(args.bdt)
suffix = args.suffix
base_plot_dir = "/share/storage/jykim/plots/MC15rd/etaKp/pipipi/generic"
base_fit_dir = f"{base_plot_dir}/fitresult"

fitresult_name_plus = (
    f"{base_fit_dir}/MC15rd_etaKp_pipipi_fit_opt_loose_v7_fitv15_bdt_"
...
if args.output:
    ...
    file_name_out = (
        f"{base_plot_dir}/MC15rd_etaKp_pipipi_fit_opt_loose_v7_fitv15_bdt_"
...
#
# The upper pad and lower-pad asymmetry must use ds_weight.  TH1::Sumw2 stores
# sum(w^2) per bin, which is then propagated to the asymmetry error below.
# -----------------------------------------------------------------------------
def sign_cut_expression(sign_name):
    if sign_name == "plus":
...
def selection_expression(sign_name, charge):
    return f"{charge_var}=={charge} && rank_Dp_chiProb==1 && {sign_cut_expression(sign_name)}"


def make_weighted_hist(name, sign_name, charge, nbins):
    # TTree::Draw fills a histogram registered in the current ROOT directory.
    # Delete any stale object with the same name, then detach after filling.
    if ROOT.gDirectory.Get(name):
        ROOT.gDirectory.Delete(f"{name};*")

    h = ROOT.TH1D(name, "", nbins, fit_range[0], fit_range[1])
    h.Sumw2()

    cut = selection_expression(sign_name, charge)
    draw_expr = f"{fit_variable}>>{name}"
    weight_expr = f"ds_weight*({cut})"
    print(f"{name}: {cut}")
    n_drawn = mychain.Draw(draw_expr, weight_expr, "goff")
    h.SetDirectory(0)
    print(f"  drawn entries = {n_drawn}, weighted integral = {h.Integral():.6g}")
    return h


h_sp_Dp = make_weighted_hist("h_sp_Dp", "plus", 1, args.nbins)
h_sp_Dm = make_weighted_hist("h_sp_Dm", "plus", -1, args.nbins)
h_sm_Dp = make_weighted_hist("h_sm_Dp", "minus", 1, args.nbins)
h_sm_Dm = make_weighted_hist("h_sm_Dm", "minus", -1, args.nbins)

print("Weighted yields:")
print("  sign plus,  D+:", h_sp_Dp.Integral())
print("  sign plus,  D-:", h_sp_Dm.Integral())
print("  sign minus, D+:", h_sm_Dp.Integral())
print("  sign minus, D-:", h_sm_Dm.Integral())
...
f_sig, result_object_sig = open_fit_result(
    "/share/storage/jykim/plots/MC15rd/etaKp/pipipi/"
    "MC15re_6M_etapip_pipipi_K_Dp_M_v12_CB_conv_result_extended_train_Dp_CMS_p.0.77.root"
)
fit_args_sig = result_object_sig.floatParsFinal()

f_sig_Ds, result_object_sig_Ds = open_fit_result(
    "/share/storage/jykim/plots/MC15rd/etaKp/pipipi/"
    "MC15rd_6M_etapip_pipipi_K_Dp_M_v12_CB_conv_result_extended_train_Dp_CMS_p_Ds.0.77.weighted.root"
)
...
f_kde_ws = ROOT.TFile.Open(
    "/share/storage/jykim/plots/MC15rd/etaKp/pipipi/"
    "MC15re_6M_etapip_pipipi_Dp_M_v12_result_true_extended_train_Dp_CMS_p.0.77_workspace.root"
)
...
def get_kde_fixed_yield(sign_name):
    f_kde, fitres_kde = open_fit_result(
        "/share/storage/jykim/plots/MC15rd/etaKp/pipipi/generic/fitresult/"
        f"MC15rd_etaKp_pipipi_fit_opt_loose_v7_fitv12_bdt_train_Dp_CMS_p_{sign_name}_0.77_only_true_KDE_weighted.root"
    )
    pars = fitres_kde.floatParsFinal()
    y = must_find(pars, "N_total").getVal()
    return f_kde, y


# -----------------------------------------------------------------------------
# Model definition: same PDF structure as etapip_pipipi_K(2).py, but tagged so
...
def build_model(tag, sign_name):
    ...
    N_total = RooRealVar(f"N_total_{tag}", "N_total", 1200 * scale, 0.0, 20000 * scale)
    ...
    N_total_Ds = RooRealVar(f"N_total_Ds_{tag}", "N_total_Ds", 8000 * scale, 0.0, 50000 * scale)
    ...
    Nbkg_total = RooRealVar(f"Nbkg_total_{tag}", "Nbkg_total", 30000 * scale, 0.0, 100000 * scale)
    Acp_bkg = RooRealVar(f"Acp_bkg_{tag}", "Acp_bkg", 0, -1.0, 1.0)
...
def make_asym_hist(hp, hm, name):
    """Build weighted asymmetry histogram.

    For each bin, P=sum(w) for D+ and M=sum(w) for D-.  With independent weighted
    sums Var(P)=sum(w^2)_P and Var(M)=sum(w^2)_M, the propagated uncertainty is

        sigma_A^2 = (2M/(P+M)^2)^2 Var(P) + (2P/(P+M)^2)^2 Var(M).

    This reduces to sqrt((1-A^2)/(P+M)) for unweighted histograms.
    """
    h = hp.Clone(name)
    h.SetDirectory(0)
    h.Reset("ICES")
    for ibin in range(1, hp.GetNbinsX() + 1):
        p = hp.GetBinContent(ibin)
        m = hm.GetBinContent(ibin)
        var_p = hp.GetBinError(ibin) ** 2
        var_m = hm.GetBinError(ibin) ** 2
        total = p + m
        if total > 0:
            asym = (p - m) / total
            err2 = (4.0 * m * m * var_p + 4.0 * p * p * var_m) / (total ** 4)
            h.SetBinContent(ibin, asym)
            h.SetBinError(ibin, math.sqrt(max(0.0, err2)))
        else:
...
# MC sign-averaged asymmetry curve is shown without blinding.
