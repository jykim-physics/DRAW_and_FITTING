...
parser = argparse.ArgumentParser(
    description=(
        "Plot combined sign-plus/sign-minus MC result for etapip_gg_K using existing "
        "weighted RooFitResult files. No refit is performed. The lower-pad asymmetry is "
        "0.5*(A_raw(sign plus) + A_raw(sign minus)), with ds_weight included in both "
        "histograms and error propagation."
    )
)
...
parser.add_argument("--nbins", type=int, default=100, help="Number of bins for weighted MC histograms")
...
parser.add_argument(
    "--suffix",
    default="KDE_fixed_2nd_cheby",
    help="Suffix used in the original weighted MC fit-result filenames, before '_weighted.root'",
)
parser.add_argument(
    "--output-suffix",
    default="",
    help="Suffix for the output plot. If empty, use '<suffix>_mass_asym_pad'.",
)
parser.add_argument("--output", default="", help="Output PDF path. If empty, use the default /share/storage/... path")
parser.add_argument("--weight-var", default="ds_weight", help="Name of the MC event-weight branch")
parser.add_argument("--weight-scale", type=float, default=1.0, help="Overall scale multiplied to the event weight")
parser.add_argument(
    "--kde-yield-scale",
    type=float,
    default=1.0,
    help="Scale applied to the fixed D+ -> eta pi+ peaking-background yield from the KDE fit",
)
parser.add_argument(
    "--blind",
    action="store_true",
    help="Hide the lower-pad asymmetry in the original etapip_gg_K blinding window. Off by default for MC.",
)
args = parser.parse_args()

BDT_cut = str(args.bdt)
suffix = args.suffix
plot_suffix = args.output_suffix if args.output_suffix else f"{suffix}_mass_asym_pad"
base_plot_dir = "/share/storage/jykim/plots/MC15rd/etaKp/gg/generic"
base_fit_dir = f"{base_plot_dir}/fitresult"

fitresult_name_plus = (
    f"{base_fit_dir}/MC15rd_etaKp_gg_fit_opt_loose_v7_fitv15_bdt_"
    f"{args.train}_plus_{BDT_cut}_{suffix}_weighted.root"
)
fitresult_name_minus = (
    f"{base_fit_dir}/MC15rd_etaKp_gg_fit_opt_loose_v7_fitv15_bdt_"
    f"{args.train}_minus_{BDT_cut}_{suffix}_weighted.root"
)
...
if args.output:
    ...
    file_name_out = (
        f"{base_plot_dir}/MC15rd_etaKp_gg_fit_opt_loose_v7_fitv15_bdt_"
        f"{args.train}_Dall_CMS_signAvg_{BDT_cut}_{plot_suffix}_weighted.pdf"
    )
...
cm_elements = [
    "15rd_jae_e7_18_4S_v3",
    "15rd_jae_e20_b26_v1",
    "15rd_jae_e20_e26_4S_v2",
    "15rd_jae_e21_5S_scan_v1",
    "15rd_jae_mori_off_v1",
]
...
event_weight = ROOT.RooRealVar(args.weight_var, args.weight_var, -1000, 1000)
...
# -----------------------------------------------------------------------------
# MC import: four weighted samples are built explicitly.  The upper-pad mass
# histogram uses the sum of ds_weight, and its bin error is sqrt(sum w^2).
# The lower-pad asymmetry error uses the same weighted-bin error propagation.
# -----------------------------------------------------------------------------
_dataset_keepalive = []
...
def make_dataset(name, sign_name, charge):
    ...
    before = ROOT.RooDataSet(
        f"{name}_before_weighting",
        f"{name}_before_weighting",
        varset,
        ROOT.RooFit.Import(mychain),
        ROOT.RooFit.Cut(cut),
    )
    weight_name = f"w_scaled_{name}"
    w_scaled = ROOT.RooFormulaVar(
        weight_name,
        weight_name,
        f"{args.weight_scale:.17g}*@0",
        ROOT.RooArgList(event_weight),
    )
    before.addColumn(w_scaled)
    weighted = ROOT.RooDataSet(name, name, before, before.get(), "", weight_name)

    print(f"  unweighted entries = {before.numEntries()}")
    print(f"  weighted sum       = {weighted.sumEntries()}")

    # Keep Python proxies alive for old PyROOT versions.
    _dataset_keepalive.extend([before, w_scaled, weighted])
    return weighted


data_sp_Dp = make_dataset("mc_signPlus_Dp_weighted", "plus", 1)
data_sp_Dm = make_dataset("mc_signPlus_Dm_weighted", "plus", -1)
data_sm_Dp = make_dataset("mc_signMinus_Dp_weighted", "minus", 1)
data_sm_Dm = make_dataset("mc_signMinus_Dm_weighted", "minus", -1)
...
def get_kde_fixed_yield(sign_name):
    ...
    y = args.kde_yield_scale * must_find(pars, "N_total").getVal()
    return f_kde, y
...
print("Loaded fitted Acp values:")
print(f"  sign plus  Acp    = {model_sp['params']['Acp'].getVal(): .6f} +/- {model_sp['params']['Acp'].getError(): .6f}")
print(f"  sign minus Acp    = {model_sm['params']['Acp'].getVal(): .6f} +/- {model_sm['params']['Acp'].getError(): .6f}")
print(f"  signAvg    Acp    = {0.5*(model_sp['params']['Acp'].getVal()+model_sm['params']['Acp'].getVal()): .6f}")
print(f"  sign plus  Acp_Ds = {model_sp['params']['Acp_Ds'].getVal(): .6f} +/- {model_sp['params']['Acp_Ds'].getError(): .6f}")
print(f"  sign minus Acp_Ds = {model_sm['params']['Acp_Ds'].getVal(): .6f} +/- {model_sm['params']['Acp_Ds'].getError(): .6f}")
print(f"  signAvg    Acp_Ds = {0.5*(model_sp['params']['Acp_Ds'].getVal()+model_sm['params']['Acp_Ds'].getVal()): .6f}")

# -----------------------------------------------------------------------------
# Histogram helpers.
# -----------------------------------------------------------------------------
def hist_from_weighted_data(data, name, nbins):
    """Build a TH1 with bin content=sum(w) and bin error=sqrt(sum(w^2))."""
    h = ROOT.TH1D(name, name, nbins, fit_range[0], fit_range[1])
    h.SetDirectory(0)
    h.Sumw2()
    for ientry in range(data.numEntries()):
        row = data.get(ientry)
        mass = row.getRealValue(fit_variable)
        weight = data.weight()
        h.Fill(mass, weight)
    return h


h_sp_Dp = hist_from_weighted_data(data_sp_Dp, "h_sp_Dp", args.nbins)
h_sp_Dm = hist_from_weighted_data(data_sp_Dm, "h_sp_Dm", args.nbins)
h_sm_Dp = hist_from_weighted_data(data_sm_Dp, "h_sm_Dp", args.nbins)
h_sm_Dm = hist_from_weighted_data(data_sm_Dm, "h_sm_Dm", args.nbins)
...
def make_asym_hist(hp, hm, name):
    """Weighted asymmetry and error propagation.

    A = (Wp - Wm)/(Wp + Wm), where Wp and Wm are the weighted bin sums.
    Vp = sum(wp^2) and Vm = sum(wm^2) are taken from TH1 bin errors.

    sigma_A^2 = (2 Wm / S^2)^2 Vp + (2 Wp / S^2)^2 Vm,
    with S = Wp + Wm.
    """
    h = hp.Clone(name)
    h.SetDirectory(0)
    ...
    for ibin in range(1, hp.GetNbinsX() + 1):
        wp = hp.GetBinContent(ibin)
        wm = hm.GetBinContent(ibin)
        vp = hp.GetBinError(ibin) ** 2
        vm = hm.GetBinError(ibin) ** 2
        total = wp + wm
        if abs(total) > 0.0:
            asym = (wp - wm) / total
            denom4 = total ** 4
            var_asym = 4.0 * (wm * wm * vp + wp * wp * vm) / denom4 if denom4 > 0 else 0.0
            h.SetBinContent(ibin, asym)
            h.SetBinError(ibin, math.sqrt(max(0.0, var_asym)))
        else:
...
canvas.Update()
#add_lumi_prelim(canvas, lumi_fb=4*428)
canvas.Update()
