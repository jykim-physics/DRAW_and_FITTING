...
parser = argparse.ArgumentParser(
    description=(
        "Plot combined sign-plus/sign-minus result for etapip_gg using existing "
...
parser.add_argument(
    "--suffix",
    default="sumw2fixed_mass_asym_pad",
    help="Suffix used in the original etapip_gg fit-result filenames",
)
parser.add_argument("--output", default="", help="Output PDF path. If empty, use the default /share/storage/... path")
parser.add_argument(
    "--no-blind",
    action="store_true",
    help="Do not hide the lower-pad asymmetry in the original etapip_gg blinding window",
)
...
suffix = args.suffix
base_plot_dir = "/share/storage/jykim/plots/proc_all/etapip/gg/generic"
base_fit_dir = f"{base_plot_dir}/fitresult"

fitresult_name_plus = (
    f"{base_fit_dir}/proc13_etapip_gg_fit_opt_loose_v7_fitv12_bdt_"
...
if args.output:
    ...
    file_name_out = (
        f"{base_plot_dir}/proc13_etapip_gg_fit_opt_loose_v7_fitv12_bdt_"
...
# Input ntuples: same logic as the original etapip_gg fit script.
...
def build_model(tag):
    # Use the same blinding offsets as the original fit script.
    random.seed(42)
    acp_bias_value = round(random.uniform(-0.1, 0.1), 10)
    random.seed(62)
    acp_ds_bias_value = round(random.uniform(-0.1, 0.1), 10)

    N_total = RooRealVar(f"N_total_{tag}", "N_total", 110000 * scale, 50000 * scale, 300000 * scale)
    Acp_blind = RooRealVar(f"Acp_blind_{tag}", "Acp_blind", 0, -0.5, 0.5)
    Acp_bias = RooRealVar(f"Acp_bias_{tag}", "Acp_bias", acp_bias_value)
    Acp_bias.setConstant(True)
    Acp = RooFormulaVar(f"Acp_{tag}", "@0+@1", RooArgList(Acp_blind, Acp_bias))
    ...
    N_total_Ds = RooRealVar(
        f"N_total_Ds_{tag}", "N_total_Ds", 210000 * scale, 80000 * scale, 520000 * scale
    )
    Acp_Ds_blind = RooRealVar(f"Acp_Ds_blind_{tag}", "Acp_Ds_blind", 0, -0.5, 0.5)
    Acp_Ds_bias = RooRealVar(f"Acp_Ds_bias_{tag}", "Acp_Ds_bias", acp_ds_bias_value)
    Acp_Ds_bias.setConstant(True)
    Acp_Ds = RooFormulaVar(f"Acp_Ds_{tag}", "@0+@1", RooArgList(Acp_Ds_blind, Acp_Ds_bias))
    ...
    Nbkg_total = RooRealVar(
        f"Nbkg_total_{tag}", "Nbkg_total", 600000 * scale, 100000 * scale, 2000000 * scale
    )
...
# Preserve the original etapip_gg blinding of the lower-pad asymmetry by default.
