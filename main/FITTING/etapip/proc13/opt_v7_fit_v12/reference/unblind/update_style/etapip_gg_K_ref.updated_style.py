...
# Create the output directories used below.
for output_path in (file_name_Dall, fitresult_name, fitresult_text):
...
def add_lumi_prelim(canvas, lumi_fb=428, is_preliminary=True):
    """Draw the Belle II and luminosity labels using the signAvg style."""
    canvas.cd(1)
    pos_x = 0.79
    top_y = 0.90
    line_spacing = 0.05
    ...
    # Keep the TLatex objects alive after this function returns.
...
frame_D_all.GetXaxis().SetTitle(fit_var_name)
...
leg1 = ROOT.TLegend(0.20, 0.65, 0.40, 0.90)
...
# -----------------------------------------------------------------------------
# Lower pad: retain the original Pull representation, changing only typography.
# -----------------------------------------------------------------------------
...
# The invariant-mass title remains on the upper pad, as in signAvg.
...
line = ROOT.TLine(fit_range[0], 0.0, fit_range[1], 0.0)
line1 = ROOT.TLine(fit_range[0], 3.0, fit_range[1], 3.0)
line2 = ROOT.TLine(fit_range[0], -3.0, fit_range[1], -3.0)
