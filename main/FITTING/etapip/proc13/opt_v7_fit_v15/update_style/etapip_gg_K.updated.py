...
suffix = "KDE_fixed_2nd_cheby_unblind_updated_style"
...
# Upper-pad typography copied from the signAvg plotting script.
# The pull-pad settings below are intentionally left unchanged.
TEXT_FONT = 42
FONT_SIZE_UP = 0.053
...
# Keep the same 100-bin RooPlot binning while adopting the signAvg axis style.
PLOT_NBINS = 100
plot_bin_width_mev = 1000.0 * (fit_range[1] - fit_range[0]) / PLOT_NBINS
frame_D_all = x.frame(ROOT.RooFit.Bins(PLOT_NBINS), ROOT.RooFit.Title(""))
frame_D_all.GetXaxis().SetTitle(fit_var_name)
frame_D_all.GetYaxis().SetTitle(f"Candidates per {plot_bin_width_mev:.2f} MeV/c^{{2}}")
...
data_hist = frame_D_all.getHist("data")
fit_curve = frame_D_all.getCurve("Fitting")

data_max = max(
    data_hist.GetPointY(i)
    for i in range(data_hist.GetN())
)

fit_max = max(
    fit_curve.GetPointY(i)
    for i in range(fit_curve.GetN())
)

max_y = max(data_max, fit_max)

frame_D_all.SetMinimum(0.0)
frame_D_all.SetMaximum(1.2 * max_y)
frame_D_all.Draw("PE")
#frame_D_all.SetMinimum(0.0)
#frame_D_all.SetMaximum(1.2 * frame_D_all.GetMaximum())
#frame_D_all.Draw("PE")
...
leg1.AddEntry("DpEtaPip", "#font[42]{D^{+} #rightarrow #eta #pi^{+}}", "l")
leg1.AddEntry("Combinatorial", "#font[42]{Combinatorial}", "f")
leg1.Draw()
