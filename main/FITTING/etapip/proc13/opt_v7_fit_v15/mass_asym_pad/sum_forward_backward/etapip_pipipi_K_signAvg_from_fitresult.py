...
parser = argparse.ArgumentParser(
    description=(
        "Plot combined sign-plus/sign-minus result for etapip_pipipi_K using existing "
...
parser.add_argument(
    "--suffix",
    default="KDE_fixed_2nd_cheby_unblind_weighted",
    help="Suffix used in the original etapip_pipipi_K fit-result filenames",
)
parser.add_argument("--output", default="", help="Output PDF path. If empty, use the default /share/storage/... path")
parser.add_argument(
    "--no-blind",
    action="store_true",
    help="Do not hide the lower-pad asymmetry in the original etapip_pipipi_K blinding window",
)
args = parser.parse_args()

BDT_cut = str(args.bdt)
suffix = args.suffix
base_plot_dir = "/share/storage/jykim/plots/proc_all/etaKp/pipipi/generic"
base_fit_dir = f"{base_plot_dir}/fitresult"

fitresult_name_plus = (
    f"{base_fit_dir}/proc13_etaKp_pipipi_fit_opt_loose_v7_fitv15_bdt_"
...
if args.output:
    ...
    file_name_out = (
        f"{base_plot_dir}/proc13_etaKp_pipipi_fit_opt_loose_v7_fitv15_bdt_"
...
def get_kde_fixed_yield(sign_name):
    ...
    y = 0.25 * 1.221 * must_find(pars, "N_total").getVal()
    return f_kde, y
