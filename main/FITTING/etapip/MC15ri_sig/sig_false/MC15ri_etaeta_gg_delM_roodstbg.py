...
ROOT.SetBelle2Style()
file_name = "/share/storage/jykim/plots/MC15ri/etaeta/gg/MC15ri_1M_etaeta_gg_delM_tight_v1_double_false_rooDstD0BG.png"
...
D0_truth_var = "D0_isSignal"
#cuts = rank_var + "==1" + " && " + truth_var + "!=1"
cuts = rank_var + "==1" + " && " + truth_var + "!=1" + " && " + D0_truth_var + "==1"
#cuts = rank_var + "==1" + " && " + truth_var + "!=1"
...
D0_truth_var = ROOT.RooRealVar(D0_truth_var, D0_truth_var, 0, 30)

# Create a TChain and add all ROOT files
mychain = ROOT.TChain(tree_name)
mychain.Add("/share/storage/jykim/storage_ghi/Ntuples_ghi_2/MC15ri_sigMC/Dtoetaeta_gg/240405_tight_v1_treefit_BCS_etapi0const/*.root")

# data = ROOT.RooDataSet("data","", ROOT.RooArgSet(x,y,z), ROOT.RooFit.Import(mychain), Cut=" D0_M>1.68 & D0_M<2.05 & Belle2Pi0Veto_75MeV > 0.022 ")
print(cuts)
before_data = ROOT.RooDataSet("data","", mychain, ROOT.RooArgSet(x,chiProb_rank,truth_var,D0_truth_var), cuts)
...
#mass = ROOT.RooRealVar("mass", "mass", 1.8, 2.0)  # Mass variable
#mean = ROOT.RooRealVar("mean", "mean", 1.865)    # Mean of the signal peak
mean = ROOT.RooRealVar("mean", "mean", 0.1455)    # Mean of the signal peak
#width = ROOT.RooRealVar("width", "width", 0.01)  # Width of the signal peak
a = ROOT.RooRealVar("a", "a", -0.1, -10, 10)     # Exponential coefficient
b = ROOT.RooRealVar("b", "b", -0.1, -10, 10)     # Exponential coefficient
c = ROOT.RooRealVar("c", "c", -0.1, -10, 10)     # Exponential coefficient
rooDstD0BG = ROOT.RooDstD0BG("rooDstD0BG", "RooDstD0BG", x, mean, c, a, b)
...
frac3 = ROOT.RooRealVar("frac3", "frac3", 0.5, 0.0, 1.0)
#model = ROOT.RooAddPdf("model", "model", ROOT.RooArgList(gaussian, gaussian2, bifurcated_gaussian, rooDstD0BG), ROOT.RooArgList(frac,frac2,frac3))
model = ROOT.RooAddPdf("model", "model", ROOT.RooArgList(bifurcated_gaussian, rooDstD0BG), ROOT.RooArgList(frac))
...
model.plotOn(frame, ROOT.RooFit.Name("rooDstD0BG"),ROOT.RooFit.Components("rooDstD0BG"), ROOT.RooFit.LineStyle(ROOT.kDashed), ROOT.RooFit.LineColor(ROOT.kGreen-2))
...
leg1.AddEntry("rooDstD0BG", "rooDstD0BG", "l")
