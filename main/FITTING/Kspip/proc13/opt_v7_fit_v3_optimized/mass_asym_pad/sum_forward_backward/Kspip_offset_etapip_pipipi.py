...
base_plot_dir = "/share/storage/jykim/plots/proc13/Kspip/pipipi/generic"
base_fit_dir = f"{base_plot_dir}/fitresult"

fitresult_name_plus = (
    f"{base_fit_dir}/proc13_Kspip_pipipi_fit_opt_loose_v7_fitv3_{args.train}_Dp_CMS_plus_{suffix}.root"
)
fitresult_name_minus = (
    f"{base_fit_dir}/proc13_Kspip_pipipi_fit_opt_loose_v7_fitv3_{args.train}_Dp_CMS_minus_{suffix}.root"
)
...
if args.output:
    ...
    file_name_out = (
        f"{base_plot_dir}/proc13_Kspip_pipipi_fit_opt_loose_v7_fitv3_Dall_{args.train}_Dp_CMS_signAvg_{suffix}.pdf"
    )
...
fit_variable = "Dp_M"
fit_var_name = "M(K_{S}^{0}#pi^{+}) [GeV/c^{2}]"
fit_range = (1.8, 1.93)
...
def build_model(tag):
    N_total = RooRealVar(f"N_total_{tag}", "N_total",  1500000 * scale, 40000 * scale, 50000000 * scale)
    ...
    mean = ROOT.RooRealVar(f"mean_{tag}", "mean", 1.87, 1.86, 1.88)
    ...
    x_bkg1_tau = ROOT.RooRealVar(f"x_bkg1_tau_{tag}", "x_bkg1_tau", -0.5, -20, 20)
...
h_data_all.GetYaxis().SetTitle(f"Candidates per {1000.0 * bin_width:.1f} MeV/c^{{2}}")
