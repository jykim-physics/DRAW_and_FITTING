...
for element in cm_elements:
    ...
    pattern = f"{base_path}/{element}/{ref_tree}/{tree_name}/0.92/weighted/weighted*.BCS.root"
...
N_total = RooRealVar("N_total", "N_total (N_D+ + N_D-)", 250000*scale, 20000*scale, 400000*scale)  # N_total = N_D+ + N_D-
Acp = RooRealVar("Acp", "Acp", 0, -0.5, 0.5)  # A_Cp as a fit parameter
...
Nbkg_total = ROOT.RooRealVar("Nbkg_total", "Number of background events for D+",  20000*scale,500*scale,80000*scale)
...
sigma = ROOT.RooRealVar("sigma", "sigma", 0.002, 0.00001, 0.01)  # Width parameter
...
x_bkg1_tau = ROOT.RooRealVar("x_bkg1_tau", "c0",-1.5, -20, 10)
