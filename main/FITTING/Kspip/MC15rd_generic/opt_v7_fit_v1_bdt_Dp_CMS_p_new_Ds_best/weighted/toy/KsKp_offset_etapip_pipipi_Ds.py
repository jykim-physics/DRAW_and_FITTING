...
for element in cm_elements:
    ...
    pattern = f"{base_path}/{element}/{ref_tree}/{tree_name}/0.90/weighted/weighted*.BCS.root"
...
N_total = RooRealVar("N_total", "N_total (N_D+ + N_D-)", 300000*scale, 20000*scale, 600000*scale)  # N_total = N_D+ + N_D-
...
Nbkg_total = ROOT.RooRealVar("Nbkg_total", "Number of background events for D+", 40000*scale,500*scale,100000*scale)
...
frame_D_plus.GetXaxis().SetTitle("M(K_{S}^{0}K^{+})")
...
frame_D_minus.GetXaxis().SetTitle("M(K_{S}^{0}K^{-})")
