...
fix_Acp_value = 0.1
#Acp.setVal(fix_Acp_value)
#Acp.setConstant(True)

Acp_const = ROOT.RooGaussian("Acp_const", "Acp_const", Acp, 0.1, 1e-8)

ToyMC_all = ROOT.RooMCStudy(sim_model, {x,cat}, ROOT.RooFit.Constrain(Acp), Extended(True), FitOptions(Save(True),PrintEvalErrors(1),PrintLevel(1), NumCPU(2), Offset("initial")))
...
toyMC_canvas = ROOT.TCanvas("toyMC_canvas", "D+ fit", 800, 600)
toyMC_frame_Acp.Draw()
toyMC_canvas.SaveAs(f"toy_Acp_{ref_tree}_{tree_name}_{args.sign}_{fix_Acp_value}.png")

toyMC_canvas_N_total = ROOT.TCanvas("toyMC_canvas_N_total", "D+ fit", 800, 600)
toyMC_frame_N_total.Draw()
toyMC_canvas_N_total.SaveAs(f"toy_N_total_{ref_tree}_{tree_name}_{args.sign}_{fix_Acp_value}.png")



toyMC_frame_Acp_val = ToyMC_all.plotParam(Acp, Bins(50))
toyMC_frame_Acp_val.GetXaxis().SetTitle("Fitted A_{CP}(D^{#pm})")

toyMC_canvas_Acp_val = ROOT.TCanvas("toyMC_canvas_Acp_val", "Acp value", 800, 600)
toyMC_frame_Acp_val.Draw()
toyMC_canvas_Acp_val.SaveAs(f"toy_Acp_val_{ref_tree}_{tree_name}_{args.sign}_{fix_Acp_value}.png")
