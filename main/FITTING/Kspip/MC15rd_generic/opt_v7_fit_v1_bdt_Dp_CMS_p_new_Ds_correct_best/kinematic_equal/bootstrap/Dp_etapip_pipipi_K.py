...
#cm_elements = ["MCrd_Ks_e7_18_4S_v3"]
...
full_var_set = ROOT.RooArgSet(x, Pip_charge, Dp_CMS_cosTheta,bdtg_norm, ds_weight)
...
def fit_and_extract_acp(data_Dp, data_Dm, full_var_set, isAfter_or_Before):
    N_total = RooRealVar("N_total", "N_total (N_D+ + N_D-)", 50000, 1000, 150000)
    ...
    Nbkg_total = RooRealVar("Nbkg_total", "Nbkg_total", 10000, 0, 100000)
...
for i in range(n_bootstrap):
    ...
    for _ in range(n_entries_Dp):
        ...
        event = data.get(idx)
        ds_w = event.getRealValue("ds_weight")
        #w_scaled = event.getRealValue("w_scaled")
        data_boot_Dp_origin.add(event, ds_w)

        #data_boot_Dp.add(data.get(idx), w_scaled)
        data_boot_Dp.add(data.get(idx), data.weight())

    for _ in range(n_entries_Dm):
        idx = random.randint(0, n_entries_Dm - 1)
        event = data_cc.get(idx)
        ds_w = event.getRealValue("ds_weight")
        #w_scaled = event.getRealValue("w_scaled")
        data_boot_Dm_origin.add(event, ds_w)

        #data_boot_Dm.add(data_cc.get(idx), w_scaled)
...
with uproot.recreate(f"bootstrap_acp_results_Dp_etaKp_pipipi_{args.sign}.root") as f:
