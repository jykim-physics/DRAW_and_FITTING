import ROOT
from ROOT import RooFit, RooRealVar, RooDataSet, RooArgList, RooAddPdf, RooGaussian, RooFormulaVar, RooSimultaneous, RooCategory, TFile, RooStats
...
BDT_cut = 0.74
suffix = f"{BDT_cut}_new_Ds_correct_float_var_{args.float_var}"
file_name_Dp = f"/share/storage/jykim/plots/MC15rd/Kspip/pipipi/generic/MC15rd_Kspip_pipipi_fit_opt_loose_v7_fitv1_Dp_{args.train}_Dp_CMS_{args.sign}_{suffix}.png"
file_name_Dm = f"/share/storage/jykim/plots/MC15rd/Kspip/pipipi/generic/MC15rd_Kspip_pipipi_fit_opt_loose_v7_fitv1_Dm_{args.train}_Dp_CMS_{args.sign}_{suffix}.png"
file_name_Dall = f"/share/storage/jykim/plots/MC15rd/Kspip/pipipi/generic/MC15rd_Kspip_pipipi_fit_opt_loose_v7_fitv1_Dall_{args.train}_Dp_CMS_{args.sign}_{suffix}.png"
fitresult_name = f"/share/storage/jykim/plots/MC15rd/Kspip/pipipi/generic/fitresult/MC15rd_Kspip_pipipi_fit_opt_loose_v7_fitv1_{args.train}_Dp_CMS_{args.sign}_{suffix}.root"
fitresult_text = f"/share/storage/jykim/plots/MC15rd/Kspip/pipipi/generic/fitresult/MC15rd_Kspip_pipipi_fit_opt_loose_v7_fitv1_{args.train}_Dp_CMS_{args.sign}_{suffix}.txt"
file_sweight = f"/share/storage/jykim/sweight/MC15rd/Kspip/pipipi/MC15rd_Kspip_pipipi_fit_opt_loose_v7_fitv1_Dp_{suffix}.root"
...
#Pip_kaonID_var = "Pip_kaonID"
Pip_kaonID_var = "Pip_pionIDNN"

#cuts_Dp = charge_var + "==1" + " & Pip_kaonID<0.01"
#cuts_Dm = charge_var + "==-1" + " & Pip_kaonID<0.01"
#cuts_Dp = charge_var + "==1" + " & Pip_pionIDNN>0.6"
#cuts_Dm = charge_var + "==-1" + " & Pip_pionIDNN>0.6"
...
Pip_kaonID = ROOT.RooRealVar(Pip_kaonID_var, Pip_kaonID_var, 0, 1)
...
before_data = ROOT.RooDataSet("data","", mychain, ROOT.RooArgSet(x,Pip_charge,Pip_kaonID, Dp_CMS_cosTheta, BDT, Pip_dr, Dp_dz, Dp_cosAngleBetweenMomentumAndVertexVectorInXYPlane, Dp_cosHelicityAngleMomentum, Dp_CMS_p), cuts_Dp)
...
for i in file_list:
    mychain_cc.Add(i)
before_data_cc = ROOT.RooDataSet("data","", mychain_cc, ROOT.RooArgSet(x,Pip_charge, Pip_kaonID, Dp_CMS_cosTheta, BDT, Pip_dr, Dp_dz, Dp_cosAngleBetweenMomentumAndVertexVectorInXYPlane, Dp_cosHelicityAngleMomentum, Dp_CMS_p), cuts_Dm)
...
N_total = RooRealVar("N_total", "N_total (N_D+ + N_D-)", 1500000*scale*CMS_scale, 500000*scale*CMS_scale, 4000000*scale*CMS_scale)  # N_total = N_D+ + N_D-
...
Nbkg_total = ROOT.RooRealVar("Nbkg_total", "Number of background events for D+", 150000*scale*CMS_scale,30000*scale*CMS_scale,300000*scale*CMS_scale)
...
Acp_sigma = ROOT.RooRealVar("Acp_sigma", "sigma", 0, -1, 1)
Acp_gamma = ROOT.RooRealVar("Acp_gamma", "gamma", 0, -1, 1)
Acp_delta = ROOT.RooRealVar("Acp_delta", "delta", 0, -1, 1)
Acp_x_bkg1_tau = ROOT.RooRealVar("Acp_x_bkg1_tau", "c0",0, -1, 1)
Acp_sigma_gauss = ROOT.RooRealVar("Acp_sigma_gauss", "Gaussian width", 0, -1, 1)

mean_plus = ROOT.RooRealVar("mean_plus", "mean", 1.86, 1.84, 1.88)
sigma_plus = ROOT.RooRealVar("sigma_plus", "sigma", 0.002, 0.0001, 0.01)
gamma_plus = ROOT.RooRealVar("gamma_plus", "gamma", 0.01, -1.0, 1.0)
delta_plus = ROOT.RooRealVar("delta_plus", "delta", 0.6, 0.001, 3.0)
x_bkg1_tau_plus = ROOT.RooRealVar("x_bkg1_tau_plus", "c0",-0.5, -20, 20)
sigma_gauss_plus = ROOT.RooRealVar("sigma_gauss_plus", "Gaussian width", 0.002, 0.00001, 0.01)

mean_minus = RooFormulaVar("mean_minus","mean_plus * (1 - Acp_mean)/(1 + Acp_mean)", RooArgList(mean_plus, Acp_mean))
sigma_minus = RooFormulaVar("sigma_minus","sigma_plus * (1 - Acp_sigma)/(1 + Acp_sigma)", RooArgList(sigma_plus, Acp_sigma))
gamma_minus = RooFormulaVar("gamma_minus","gamma_plus * (1 - Acp_gamma)/(1 + Acp_gamma)", RooArgList(gamma_plus, Acp_gamma))
delta_minus = RooFormulaVar("delta_minus","delta_plus * (1 - Acp_delta)/(1 + Acp_delta)", RooArgList(delta_plus, Acp_delta))
...
sigma_gauss_minus = RooFormulaVar("sigma_gauss_minus","sigma_gauss_plus * (1 - Acp_sigma_gauss)/(1 + Acp_sigma_gauss)", RooArgList(sigma_gauss_plus, Acp_sigma_gauss))
...
if args.float_var == "mean":
    johnson_plus = ROOT.RooJohnson("johnson_plus", "double-sided Crystal Ball using Johnson SU", x,  mean_plus, sigma, gamma, delta)
    johnson_minus = ROOT.RooJohnson("johnson_minus", "double-sided Crystal Ball using Johnson SU", x,  mean_minus, sigma, gamma, delta)
    gauss = ROOT.RooGaussian("gauss", "Gaussian PDF", x, mean_gauss, sigma_gauss)
    sig_model_plus = ROOT.RooFFTConvPdf("sig_model_plus", "Convolution of Johnson SU and Gaussian", x, johnson_plus, gauss)
    sig_model_minus = ROOT.RooFFTConvPdf("sig_model_minus", "Convolution of Johnson SU and Gaussian", x, johnson_minus, gauss)
    ...
    model_D_plus = ROOT.RooAddPdf("model_D_plus", "D+ model",
                                  ROOT.RooArgList(sig_model_plus,  model_bkg),
                                  ROOT.RooArgList(Nsig_D_plus,  Nbkg_D_plus))
    model_D_minus = ROOT.RooAddPdf("model_D_minus", "D- model",
                                  ROOT.RooArgList(sig_model_minus,  model_bkg),
                                  ROOT.RooArgList(Nsig_D_minus,  Nbkg_D_minus))
elif args.float_var == "sigma":
    johnson_plus = ROOT.RooJohnson("johnson_plus", "double-sided Crystal Ball using Johnson SU", x,  mean, sigma_plus, gamma, delta)
    johnson_minus = ROOT.RooJohnson("johnson_minus", "double-sided Crystal Ball using Johnson SU", x,  mean, sigma_minus, gamma, delta)
    gauss = ROOT.RooGaussian("gauss", "Gaussian PDF", x, mean_gauss, sigma_gauss)
    sig_model_plus = ROOT.RooFFTConvPdf("sig_model_plus", "Convolution of Johnson SU and Gaussian", x, johnson_plus, gauss)
    sig_model_minus = ROOT.RooFFTConvPdf("sig_model_minus", "Convolution of Johnson SU and Gaussian", x, johnson_minus, gauss)
    ...
    model_D_minus = ROOT.RooAddPdf("model_D_minus", "D- model",
                                  ROOT.RooArgList(sig_model_minus,  model_bkg),
                                  ROOT.RooArgList(Nsig_D_minus,  Nbkg_D_minus))
elif args.float_var == "gamma":
    johnson_plus = ROOT.RooJohnson("johnson_plus", "double-sided Crystal Ball using Johnson SU", x,  mean, sigma, gamma_plus, delta)
    johnson_minus = ROOT.RooJohnson("johnson_minus", "double-sided Crystal Ball using Johnson SU", x,  mean, sigma, gamma_minus, delta)
    gauss = ROOT.RooGaussian("gauss", "Gaussian PDF", x, mean_gauss, sigma_gauss)
    sig_model_plus = ROOT.RooFFTConvPdf("sig_model_plus", "Convolution of Johnson SU and Gaussian", x, johnson_plus, gauss)
    sig_model_minus = ROOT.RooFFTConvPdf("sig_model_minus", "Convolution of Johnson SU and Gaussian", x, johnson_minus, gauss)
    ...
    model_D_minus = ROOT.RooAddPdf("model_D_minus", "D- model",
                                  ROOT.RooArgList(sig_model_minus,  model_bkg),
                                  ROOT.RooArgList(Nsig_D_minus,  Nbkg_D_minus))
elif args.float_var == "delta":
    johnson_plus = ROOT.RooJohnson("johnson_plus", "double-sided Crystal Ball using Johnson SU", x,  mean, sigma, gamma, delta_plus)
    johnson_minus = ROOT.RooJohnson("johnson_minus", "double-sided Crystal Ball using Johnson SU", x,  mean, sigma, gamma, delta_minus)
    gauss = ROOT.RooGaussian("gauss", "Gaussian PDF", x, mean_gauss, sigma_gauss)
    sig_model_plus = ROOT.RooFFTConvPdf("sig_model_plus", "Convolution of Johnson SU and Gaussian", x, johnson_plus, gauss)
    sig_model_minus = ROOT.RooFFTConvPdf("sig_model_minus", "Convolution of Johnson SU and Gaussian", x, johnson_minus, gauss)
    ...
    model_D_minus = ROOT.RooAddPdf("model_D_minus", "D- model",
                                  ROOT.RooArgList(sig_model_minus,  model_bkg),
                                  ROOT.RooArgList(Nsig_D_minus,  Nbkg_D_minus))
elif args.float_var == "sigma_gauss":
    johnson = ROOT.RooJohnson("johnson", "double-sided Crystal Ball using Johnson SU", x,  mean, sigma, gamma, delta)
    gauss_plus = ROOT.RooGaussian("gauss_plus", "Gaussian PDF", x, mean_gauss, sigma_gauss_plus)
    gauss_minus = ROOT.RooGaussian("gauss_minus", "Gaussian PDF", x, mean_gauss, sigma_gauss_minus)
    sig_model_plus = ROOT.RooFFTConvPdf("sig_model_plus", "Convolution of Johnson SU and Gaussian", x, johnson, gauss_plus)
    sig_model_minus = ROOT.RooFFTConvPdf("sig_model_minus", "Convolution of Johnson SU and Gaussian", x, johnson, gauss_minus)
    ...
    model_bkg_plus = ROOT.RooExponential("model_bkg_plus", "x_bkg1", x, x_bkg1_tau_plus)
    model_bkg_minus = ROOT.RooExponential("model_bkg_minus", "x_bkg1", x, x_bkg1_tau_minus)

    # Define extended PDFs for D+ and D-
    model_D_plus = ROOT.RooAddPdf("model_D_plus", "D+ model",
                                  ROOT.RooArgList(sig_model,  model_bkg_plus),
                                  ROOT.RooArgList(Nsig_D_plus,  Nbkg_D_plus))
    model_D_minus = ROOT.RooAddPdf("model_D_minus", "D- model",
                                  ROOT.RooArgList(sig_model,  model_bkg_minus),
                                  ROOT.RooArgList(Nsig_D_minus,  Nbkg_D_minus))
...
data_combined = RooDataSet("data_combined", "Combined data", RooArgList(x, w_1, BDT, Pip_dr, Dp_dz, Dp_cosAngleBetweenMomentumAndVertexVectorInXYPlane, Dp_cosHelicityAngleMomentum, Dp_CMS_p), RooFit.Index(cat),
