...
BDT_cut = 0.92
suffix = f"{BDT_cut}_new_Ds_correct_alternative_CB_gauss_wDpCMSp"
...
N_total = RooRealVar("N_total", "N_total (N_D+ + N_D-)", 250000*scale, 20000*scale, 1000000*scale)  # N_total = N_D+ + N_D-
...
Nbkg_total = ROOT.RooRealVar("Nbkg_total", "Number of background events for D+", 200000*scale,500*scale,3000000*scale)
...
alphaL = ROOT.RooRealVar("alphaL", "alphaL", 0.5, 0.0, 2.0)
nL = ROOT.RooRealVar("nL", "nL", 3.0, 0.0, 5.0)
alphaR = ROOT.RooRealVar("alphaR", "alphaR", 0.3, 0.0, 2.0)
...
sig_model = ROOT.RooFFTConvPdf("sig_model", "Convolution of Johnson SU and Gaussian", x, CB, gauss)
