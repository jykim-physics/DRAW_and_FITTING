...
file_name_Dp = "/share/storage/jykim/plots/MC15rd/KsKp/gg/generic/MC15rd_Kspip_gg_K_fit_opt_loose_v7_fitv1_Dp_all.png"
file_name_Dm = "/share/storage/jykim/plots/MC15rd/KsKp/gg/generic/MC15rd_Kspip_gg_K_fit_opt_loose_v7_fitv1_Dm_all.png"
fitresult_name = "/share/storage/jykim/plots/MC15rd/KsKp/gg/generic/fitresult/MC15rd_Kspip_gg_K_fit_opt_loose_v7_fitv1_all.root"
fitresult_text = "/share/storage/jykim/plots/MC15rd/KsKp/gg/generic/fitresult/MC15rd_Kspip_gg_K_fit_opt_loose_v7_fitv1_all.txt"
...
N_total_Ds = RooRealVar("N_total_Ds", "N_total (N_D+ + N_D-)", 200000*scale, 100000*scale, 400000*scale)  # N_total = N_D+ + N_D-
Acp_Ds = RooRealVar("Acp_Ds", "Acp", 0, -1, 1)  # A_Cp as a fit parameter

# Use Acp and N_total to define the expected signal yields for D+ and D-
Nsig_D_plus_Ds = RooFormulaVar("Nsig_D_plus_Ds",
    "0.5 * N_total_Ds * (1 + Acp_Ds)",
    RooArgList(N_total_Ds, Acp_Ds))

Nsig_D_minus_Ds = RooFormulaVar("Nsig_D_minus_Ds",
    "0.5 * N_total_Ds * (1 - Acp_Ds)",
    RooArgList(N_total_Ds, Acp_Ds))
...
mean_Ds = ROOT.RooRealVar("mean_Ds", "mean", 1.96, 1.94, 1.98)  # Central value
sigma_Ds = ROOT.RooRealVar("sigma_Ds", "sigma", 0.001, 0.0001, 0.1)  # Width parameter
gamma_Ds = ROOT.RooRealVar("gamma_Ds", "gamma", 0.0001, 0.0, 2.0)  # Skewness parameter
delta_Ds = ROOT.RooRealVar("delta_Ds", "delta", 0.7, 0.01, 3.0)  # Shape parameter

johnson_Ds = ROOT.RooJohnson("johnson_Ds", "double-sided Crystal Ball using Johnson SU", x, mean_Ds, sigma_Ds, gamma_Ds, delta_Ds)
mean_gauss_Ds = ROOT.RooRealVar("mean_gauss_Ds", "Gaussian mean", 0.0)  # Convolution will center the Gaussian at zero
sigma_gauss_Ds = ROOT.RooRealVar("sigma_gauss_Ds", "Gaussian width", 0.002, 0.00001, 0.1)
gauss_Ds = ROOT.RooGaussian("gauss_Ds", "Gaussian PDF", x, mean_gauss_Ds, sigma_gauss_Ds)

sig_model_Ds = ROOT.RooFFTConvPdf("sig_model_Ds", "Convolution of Johnson SU and Gaussian", x, johnson_Ds, gauss_Ds)
...
model_D_plus = ROOT.RooAddPdf("model_D_plus", "D+ model",
                              ROOT.RooArgList(sig_model, sig_model_Ds, model_bkg),
                              ROOT.RooArgList(Nsig_D_plus,  Nsig_D_plus_Ds, Nbkg_D_plus))
model_D_minus = ROOT.RooAddPdf("model_D_minus", "D- model",
                              ROOT.RooArgList(sig_model, sig_model_Ds,  model_bkg),
                              ROOT.RooArgList(Nsig_D_minus,  Nsig_D_plus_Ds, Nbkg_D_minus))
