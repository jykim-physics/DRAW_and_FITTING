...
modes = {
    "etapip_gg_K": [-1, 1],
    #"etapip_pipipi_K": [-10, 10],
    #"etapip_pipipi": [-0.1, 0.1],
    "etapip_gg": [-0.1, 0.1]
}

branches = ["delta_acp", "acp_peak_bkg_after"]
...
for mode, (range_min, range_max) in modes.items():
    for branch_name in branches:
        ...
        axes[0].text(0.05, 0.95, f"Mean = {mean1:.6f}%\nStd = {std1:.6f}%", transform=axes[0].transAxes,
        ...
        axes[1].text(0.05, 0.95, f"Mean = {mean2:.6f}%\nStd = {std2:.6f}%", transform=axes[1].transAxes,
                     verticalalignment='top', bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))
        ...
        # Print statistics
        print(f"cosθ* < 0: mean = {mean1:.6f}%, std = {std1:.6f}%")
        print(f"cosθ* > 0: mean = {mean2:.6f}%, std = {std2:.6f}%")

        combined_mean = (mean1 + mean2) / 2
        combined_std = np.sqrt(std1**2 + std2**2) / 2

        print(f"Combined mean: {combined_mean:.6f}%")
