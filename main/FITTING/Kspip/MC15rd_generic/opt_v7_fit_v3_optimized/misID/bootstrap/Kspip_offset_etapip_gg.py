...
N_total_var = RooRealVar("N_total_var", "N_total (N_D+ + N_D-)", 2000000*scale*CMS_scale, 40000*scale*CMS_scale, 50000000*scale*CMS_scale)  # N_total = N_D+ + N_D-
...
Nsig_D_minus_var = RooFormulaVar("Nsig_D_minus_var",
    "0.5 * N_total_var * (1 - Acp_var)",
    RooArgList(N_total_var, Acp_var))
Nbkg_total_var = ROOT.RooRealVar("Nbkg_total_var", "Number of background events for D+",400000*scale*CMS_scale,10000*scale*CMS_scale,10000000*scale*CMS_scale)
...
KDE_fit_result_var = ROOT.TFile.Open(f"/share/storage/jykim/plots/MC15rd/Kspip/gg/KDE_MC15rd_Kspip_gg_kaon_misID_Dp_M_v3_result_true_extended_train_Dp_CMS_p.0.80.{args.sign}.root")
...
f_in_var = ROOT.TFile(f"/share/storage/jykim/plots/MC15rd/Kspip/gg/KDE_MC15rd_Kspip_gg_kaon_misID_Dp_M_v3_result_true_extended_train_Dp_CMS_p.0.80.{args.sign}_workspace.root", "READ")
...
def var_fit_extract_Acp(data_Dp, data_Dm, n_bootstrap):
  mean_var = ROOT.RooRealVar("mean_var", "mean", 1.87, 1.86, 1.88)  # Central value
