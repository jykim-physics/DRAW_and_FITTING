...
file_name_Dall = f"/share/storage/jykim/plots/MC15rd/Kspip/pipipi/generic/MC15rd_Kspip_pipipi_fit_opt_loose_v7_fitv3_Dall_{args.train}_Dp_CMS_{args.sign}_{suffix}.png"
...
cuts_Dp += " && " + Dp_CMS_cosTheta_cut + " && !(etapip_Eta_isSignal == 1 && etapip_Eta_genMotherID == Pip_genMotherID && Pip_genMotherPDG==431 && Pip_mcPDG==321)"
cuts_Dm += " && " + Dp_CMS_cosTheta_cut + " && !(etapip_Eta_isSignal == 1 && etapip_Eta_genMotherID == Pip_genMotherID && Pip_genMotherPDG==-431 && Pip_mcPDG==-321)"
...
data = ROOT.RooDataSet("data","", mychain, ROOT.RooArgSet(x,Pip_charge, Dp_CMS_cosTheta, BDT,etapip_Eta_Easym,  Pip_dr, Dp_dz, Dp_cosAngleBetweenMomentumAndVertexVectorInXYPlane, Dp_cosHelicityAngleMomentum, Dp_CMS_p, rank_Dp_chiProb, etapip_Eta_isSignal, etapip_Eta_genMotherID, Pip_genMotherID, Pip_genMotherPDG, Pip_mcPDG), cuts_Dp)
...
for i in file_list:
    mychain_cc.Add(i)
data_cc = ROOT.RooDataSet("data","", mychain_cc, ROOT.RooArgSet(x,Pip_charge, Dp_CMS_cosTheta, BDT,etapip_Eta_Easym, Pip_dr, Dp_dz, Dp_cosAngleBetweenMomentumAndVertexVectorInXYPlane, Dp_cosHelicityAngleMomentum, Dp_CMS_p, rank_Dp_chiProb, etapip_Eta_isSignal, etapip_Eta_genMotherID, Pip_genMotherID, Pip_genMotherPDG, Pip_mcPDG), cuts_Dm)

#data.append(data_cc)
Num_total_cc = data_cc.sumEntries()
print(Num_total_cc)

N_total = RooRealVar("N_total", "N_total (N_D+ + N_D-)", 12000000*scale*CMS_scale, 500000*scale*CMS_scale, 10000000*scale*CMS_scale)  # N_total = N_D+ + N_D-
...
Nbkg_total = ROOT.RooRealVar("Nbkg_total", "Number of background events for D+", 30000000*scale*CMS_scale,50000*scale*CMS_scale,10000000*scale*CMS_scale)
