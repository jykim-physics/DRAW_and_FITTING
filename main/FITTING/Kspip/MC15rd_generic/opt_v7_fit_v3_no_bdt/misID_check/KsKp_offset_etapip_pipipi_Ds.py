...
suffix = "no_bdt_new_Ds_correct_removed_DpKsPip"
...
file_name_Dall = f"/share/storage/jykim/plots/MC15rd/KsKp/pipipi/generic/MC15rd_Kspip_pipipi_K_fit_opt_loose_v7_fitv3_Ds_Dall_{args.train}_Dp_CMS_{args.sign}_{suffix}.png"
...
cuts_Dp += " && " + Dp_CMS_cosTheta_cut + " && !(etapip_Eta_isSignal == 1 && etapip_Eta_genMotherID == Pip_genMotherID && Pip_genMotherPDG==411 && Pip_mcPDG==211)"
cuts_Dm += " && " + Dp_CMS_cosTheta_cut + " && !(etapip_Eta_isSignal == 1 && etapip_Eta_genMotherID == Pip_genMotherID && Pip_genMotherPDG==-411 && Pip_mcPDG==-211)"
...
etapip_Eta_isSignal = ROOT.RooRealVar("etapip_Eta_isSignal", "", -1000, 1000)
etapip_Eta_genMotherID = ROOT.RooRealVar("etapip_Eta_genMotherID", "", -1e9, 1e9)
Pip_genMotherID = ROOT.RooRealVar("Pip_genMotherID", "", -1e9, 1e9)
Pip_genMotherPDG = ROOT.RooRealVar("Pip_genMotherPDG", "", -1e9, 1e9)
Pip_mcPDG = ROOT.RooRealVar("Pip_mcPDG", "", -1e9, 1e9)

data = ROOT.RooDataSet("data","", mychain, ROOT.RooArgSet(x,Pip_charge, Dp_CMS_cosTheta, BDT, Pip_dr, Dp_dz, Dp_cosAngleBetweenMomentumAndVertexVectorInXYPlane,  Dp_cosHelicityAngleMomentum, Dp_CMS_p, rank_Dp_chiProb, etapip_Eta_isSignal, etapip_Eta_genMotherID, Pip_genMotherID, Pip_genMotherPDG, Pip_mcPDG), cuts_Dp)
...
for i in file_list:
    mychain_cc.Add(i)
data_cc = ROOT.RooDataSet("data","", mychain_cc, ROOT.RooArgSet(x,Pip_charge,Dp_CMS_cosTheta, BDT, Pip_dr, Dp_dz, Dp_cosAngleBetweenMomentumAndVertexVectorInXYPlane,  Dp_cosHelicityAngleMomentum, Dp_CMS_p, rank_Dp_chiProb, etapip_Eta_isSignal, etapip_Eta_genMotherID, Pip_genMotherID, Pip_genMotherPDG, Pip_mcPDG), cuts_Dm)

#data.append(data_cc)
Num_total_cc = data_cc.sumEntries()
print(Num_total_cc)

N_total = RooRealVar("N_total", "N_total (N_D+ + N_D-)", 800000*scale, 20000*scale, 1500000*scale)  # N_total = N_D+ + N_D-
...
Nbkg_total = ROOT.RooRealVar("Nbkg_total", "Number of background events for D+", 1600000*scale,500*scale,3000000*scale)
