...
h_data = ROOT.TH1F("h_data", "Data Histogram", n_bins, min_bin, max_bin)
h_MC   = ROOT.TH1F("h_MC",   "MC Histogram",   n_bins, min_bin, max_bin)
...
for i in range(1, h_weights.GetNbinsX() + 1):
    ...
    if content < 0:
        print(f"⚠️ Warning: Negative weight ratio found in Bin {i} ({content:.4f}). Forcing to 0.0.")
        h_weights.SetBinContent(i, 0.0)
