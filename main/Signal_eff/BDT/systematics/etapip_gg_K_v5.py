...
#DS_WEIGHTED_DIR = f"{base_path_sig}/{sig_MC_name}"
...
DATASETS = {
    ...
    "Ds_sig": {
        "elements": ["DsptoetaKp_gg", "DsptoetaKp_gg_cc"],  # 사용 안 해도 무방(override)
        "tree": "etapip_gg_K",
...
for key, cfg in DATASETS.items():
    ...
    df = load_df(files, cfg["tree"], branches_gg, weight_branch=cfg.get("weight_branch"))
    if df.empty:
...
plt.legend(ncol=2)
plt.tight_layout()
plt.savefig("BDT_output_gg.png", dpi=200)
# plt.show()
...
plt.tight_layout()
plt.savefig('eff_scan_individual_gg.png', dpi=200)
...
plt.legend()
plt.tight_layout()
plt.savefig('eff_ratio_scan_both_with_err_gg.png', dpi=200)
