...
# 5) 플롯: 효율비(ref/sig) vs BDT cut (오차 포함)
plt.figure(figsize=(6.4, 4.8))
plt.errorbar(thresholds, mc_ratio, yerr=mc_ratio_err, fmt='o', capsize=3, label=r'D$^+$: ref/sig', color='C0')
...
plt.legend()
plt.tight_layout()
plt.savefig('eff_ratio_scan_with_err.png', dpi=200)
# plt.show()

# D+ 와 Ds+ 각각 계산 (이미 위에서 비슷하게 했으니 같은 방식 반복)
def calc_ratio_with_err(df_sig, df_ref):
    ratios, ratio_errs = [], []
    for thr in thresholds:
        e_sig, de_sig = eff_binom(df_sig, thr)
        e_ref, de_ref = eff_binom(df_ref, thr)
        if (e_sig > 0) and (e_ref > 0):
            r = e_ref / e_sig
            dr = r * np.sqrt( (de_ref/e_ref)**2 + (de_sig/e_sig)**2 )
        else:
            r, dr = np.nan, np.nan
        ratios.append(r)
        ratio_errs.append(dr)
    return ratios, ratio_errs

ratio_Dp, ratio_Dp_err   = calc_ratio_with_err(df_signal_SR,    df_ref_SR)
ratio_Dsp, ratio_Dsp_err = calc_ratio_with_err(df_signal_Dsp_SR,df_ref_Dsp_SR)

plt.figure(figsize=(6.4, 4.8))
plt.errorbar(thresholds, ratio_Dp,  yerr=ratio_Dp_err,  fmt='o', capsize=3, label=r'D$^+$: ref/sig')
plt.errorbar(thresholds, ratio_Dsp, yerr=ratio_Dsp_err, fmt='s', capsize=3, label=r'D$_s^+$: ref/sig')
plt.axvline(cut0, linestyle='--', linewidth=1, color='gray', label=f'cut = {cut0:.2f}')
plt.xlabel('BDT cut')
plt.ylabel('Efficiency ratio (ref / sig)')
plt.grid(True, alpha=0.3)
plt.legend()
plt.tight_layout()
plt.savefig('eff_ratio_scan_both_with_err.png', dpi=200)
# plt.show()
