...
def calc_acp_with_weights(df, weight_column=None, global_scale=0.25):
    if weight_column:
    ...
    # Errors: sqrt(sum)
    #err_plus = np.sqrt((df_plus["__weight__"] ).sum())
    #err_minus = np.sqrt((df_minus["__weight__"] ).sum())
...
def calc_acp_cosTheta_symmetric(df, cosTheta_var="Dp_CMS_cosTheta", weight_column=None, global_scale=0.25):
