...
# -----------------------------------------
# (A) 이벤트 단위 BCS 후보 선정 by Dp_chiProb
# -----------------------------------------
# NaN 안전 처리 (최솟값/최댓값 계산이 가능하도록)
if BCS_PICK == 'lowest':
    key_series = df_all['Dp_chiProb'].fillna(np.inf)   # 낮은 값 선택 시, NaN은 배제되도록 inf로
    bcs_index = key_series.groupby(df_all[event_cols].apply(tuple, axis=1)).idxmin()
elif BCS_PICK == 'highest':
    key_series = df_all['Dp_chiProb'].fillna(-np.inf)  # 높은 값 선택 시, NaN은 배제되도록 -inf로
...
# 3) Signal Chosen Among Multi-Cands (chiProb-BCS가 신호를 택한 비율)
#    다중후보 & 신호포함 이벤트 집합에서, bcs_by_chiProb 로 선택된 하나의 후보가 신호인지 체크
chosen_per_event_df = df_all[df_all['bcs_by_chiProb']].copy()  # 이벤트당 1행
...
# 4) Overall BCS Efficiency (chiProb-BCS가 신호 이벤트에서 신호후보를 택한 비율)
...
print("=== BCS Performance Metrics (chiProb-based, pick: {}) ===".format(BCS_PICK))
...
plt.tight_layout()
plt.savefig('multiplicity_hist.png', dpi=160)
plt.show()

print("[INFO] Saved histogram as 'multiplicity_hist.png'")
