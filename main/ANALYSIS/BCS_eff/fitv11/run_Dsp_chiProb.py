...
# 아래 네 블록 중 현재 마지막 블록만 유효합니다. 필요 블록 하나만 남겨 쓰세요.
tree_name = "etapip_gg"
...
# --- chiProb BCS 규칙: 'lowest' 또는 'highest' ---
...
if tree_name == "etapip_gg_K" or tree_name == "etapip_pipipi_K":
    ...
    for element in elements:
        pattern = f"{base_path}/{element}/250216_loose_v7/{tree_name}/ref/no_BCS/{BDT_cut}/weighted/*.no_BCS.root"
        file_list += glob.glob(pattern)
#for element in elements:
#    pattern = f"{base_path}/{element}/250216_loose_v7/{tree_name}/no_BCS/{BDT_cut}/weighted/*.no_BCS.root"
#    file_list += glob.glob(pattern)
...
branches_all = [
    ...
    "multiplicity", "Dp_M", "Dp_chiProb", "ds_weight",
    # signal_cut에 필요한 브랜치
...
for file_name in file_list:
    with uproot.open(file_name) as file:
        tree = file[tree_name]
        avail = [b for b in branches_all if b in tree.keys()]
        df_temp = tree.arrays(avail, library="pd")
        dfs.append(df_temp)
...
# 필수 컬럼 체크
need_cols = ["Dp_chiProb", "ds_weight", "multiplicity"]
...
# =========================
# chiProb 기반 BCS 후보 선택
# =========================
if BCS_PICK == 'lowest':
    key_series = df_all['Dp_chiProb'].fillna(np.inf)
...
df_all['bcs_by_chiProb'] = False
df_all.loc[bcs_index, 'bcs_by_chiProb'] = True
...
print("=== Weighted BCS Performance Metrics (Ds, chiProb-based, pick: {}) ===".format(BCS_PICK))
...
plt.tight_layout()
plt.savefig('multiplicity_hist_unweighted.png', dpi=160)
plt.close()
...
plt.tight_layout()
plt.savefig('multiplicity_hist_weighted.png', dpi=160)
plt.close()

print("[INFO] Saved histograms: multiplicity_hist_unweighted.png, multiplicity_hist_weighted.png")
