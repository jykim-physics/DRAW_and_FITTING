...
def pass_ds_signal_truth(entry, daughter_abs_pdg: int) -> bool:
    """
    Ds signal truth matching.

    For pion mode: daughter_abs_pdg = 211
    For kaon mode: daughter_abs_pdg = 321
    """
    eta_is_signal = safe_int(getattr(entry, "etapip_Eta_isSignal"), nan_value=-1)
    pip_gen_mother_id = safe_int(getattr(entry, "Pip_genMotherID"), nan_value=-999999)
    eta_gen_mother_id = safe_int(getattr(entry, "etapip_Eta_genMotherID"), nan_value=-999999)
    pip_gen_mother_pdg = safe_int(getattr(entry, "Pip_genMotherPDG"), nan_value=0)
    pip_mc_pdg = safe_int(getattr(entry, "Pip_mcPDG"), nan_value=0)

    return (
        eta_is_signal == 1
        and pip_gen_mother_id == eta_gen_mother_id
        and (
            (pip_gen_mother_pdg == 431 and pip_mc_pdg == daughter_abs_pdg)
            or (pip_gen_mother_pdg == -431 and pip_mc_pdg == -daughter_abs_pdg)
        )
    )
...
def fill_all_hists_with_cut(
    ...
    for entry in chain:
        ...
        if not pass_ds_signal_truth(entry, daughter_abs_pdg=daughter_abs_pdg):
            continue

        weight = float(getattr(entry, weight_branch))
        if ROOT.TMath.IsNaN(weight):
            n_weight_nan += 1
            continue
    ...
    return n_seen, n_passed_cut, n_weight_nan, n_value_nan
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--weight-branch", default="ds_weight")
...
def main() -> int:
    ...
    print(f"[INFO] Sample 1 plus: label={args.sample1_label}, tree={args.sample1_plus_tree}, files={n_s1_plus_files}, entries={sample1_plus_chain.GetEntries()}, cut=(Pip_charge == 1 && Ds truth match with h=pi)")
    print(f"[INFO] Sample 1 cc  : label={args.sample1_label}, tree={args.sample1_cc_tree}, files={n_s1_cc_files}, entries={sample1_cc_chain.GetEntries()}, cut=(Pip_charge == -1 && Ds truth match with h=pi)")
    print(f"[INFO] Sample 2 plus: label={args.sample2_label}, tree={args.sample2_plus_tree}, files={n_s2_plus_files}, entries={sample2_plus_chain.GetEntries()}, cut=(Pip_charge == 1 && Ds truth match with h=K)")
    print(f"[INFO] Sample 2 cc  : label={args.sample2_label}, tree={args.sample2_cc_tree}, files={n_s2_cc_files}, entries={sample2_cc_chain.GetEntries()}, cut=(Pip_charge == -1 && Ds truth match with h=K)")
    print(f"[INFO] Histogram weight branch: {args.weight_branch}")
    ...
    s1_plus_seen, s1_plus_passed, s1_plus_weight_nan, s1_plus_value_nan = fill_all_hists_with_cut(
        sample1_plus_chain, h1_by_var, required_charge=1, daughter_abs_pdg=211, weight_branch=args.weight_branch
    )
    s1_cc_seen, s1_cc_passed, s1_cc_weight_nan, s1_cc_value_nan = fill_all_hists_with_cut(
        sample1_cc_chain, h1_by_var, required_charge=-1, daughter_abs_pdg=211, weight_branch=args.weight_branch
    )
    s2_plus_seen, s2_plus_passed, s2_plus_weight_nan, s2_plus_value_nan = fill_all_hists_with_cut(
        sample2_plus_chain, h2_by_var, required_charge=1, daughter_abs_pdg=321, weight_branch=args.weight_branch
    )
    s2_cc_seen, s2_cc_passed, s2_cc_weight_nan, s2_cc_value_nan = fill_all_hists_with_cut(
        sample2_cc_chain, h2_by_var, required_charge=-1, daughter_abs_pdg=321, weight_branch=args.weight_branch
    )

    print(f"[INFO] Passed sample1 pion-mode Ds truth cut: plus {s1_plus_passed}/{s1_plus_seen}, cc {s1_cc_passed}/{s1_cc_seen}, total {s1_plus_passed + s1_cc_passed}")
    print(f"[INFO] Passed sample2 kaon-mode Ds truth cut: plus {s2_plus_passed}/{s2_plus_seen}, cc {s2_cc_passed}/{s2_cc_seen}, total {s2_plus_passed + s2_cc_passed}")
    print(f"[INFO] NaN {args.weight_branch} skipped:")
    print(f"       sample1 plus={s1_plus_weight_nan}, sample1 cc={s1_cc_weight_nan}, sample2 plus={s2_plus_weight_nan}, sample2 cc={s2_cc_weight_nan}")
    print("[INFO] NaN plotted variable values skipped across all histograms:")
    print(f"       sample1 plus={s1_plus_value_nan}, sample1 cc={s1_cc_value_nan}, sample2 plus={s2_plus_value_nan}, sample2 cc={s2_cc_value_nan}")
